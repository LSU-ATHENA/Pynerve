...
ErrorResult<std::vector<uint8_t>> ArrowSerializer::serialize(const void *data, size_t size,
                                                             const SerializationContext &context)
{
    std::vector<uint8_t> result;
    const uint8_t arrow_magic[] = {0xFF, 0xFF, 'A', 'R', 'R', 'O', 'W', '0'};
    result.insert(result.end(), arrow_magic, arrow_magic + 8);
    ...
    if (size > 0)
    {
        if (data == nullptr)
        {
            return ErrorResult<std::vector<uint8_t>>::error(ErrorCode::E41_RESOURCE_LIMIT,
                                                            "Invalid Arrow data: null payload");
        }
...
ArrowSerializer::serializeWithMetadata(const void *data, size_t size,
                                       ...
                                       const SerializationContext &context)
{
    ...
    {
        return ErrorResult<std::vector<uint8_t>>::error(ErrorCode::E41_RESOURCE_LIMIT,
                                                        "Invalid Arrow metadata: too large");
    }
...
ErrorResult<std::vector<uint8_t>> ArrowSerializer::deserialize(const std::vector<uint8_t> &data,
                                                               const SerializationContext &context)
{
    if (data.size() < 12)
    {
        return ErrorResult<std::vector<uint8_t>>::error(ErrorCode::E41_RESOURCE_LIMIT,
                                                        "Invalid Arrow data: too small");
    }
    const uint8_t expected_magic[] = {0xFF, 0xFF, 'A', 'R', 'R', 'O', 'W', '0'};
    if (std::memcmp(data.data(), expected_magic, 8) != 0)
    {
        return ErrorResult<std::vector<uint8_t>>::error(ErrorCode::E31_SCHEMA_VERSION,
                                                        "Invalid Arrow magic number");
    }
    ...
    std::vector<uint8_t> result(ptr, data.data() + data.size());
    return ErrorResult<std::vector<uint8_t>>::success(std::move(result));
}
ErrorResult<std::pair<std::vector<uint8_t>, SchemaMetadata>>
ArrowSerializer::deserializeWithMetadata(const std::vector<uint8_t> &data,
                                         const SerializationContext &context)
{
...
std::vector<uint8_t> ArrowSerializer::packMetadata(const SchemaMetadata &metadata) const
{
    std::string json = "{";
...
SchemaMetadata ArrowSerializer::unpackMetadata(const std::vector<uint8_t> &metadataData) const
{
