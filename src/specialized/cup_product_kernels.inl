 ...
 */
__global__ void cupProductKernel(const int *__restrict__ cochain1_simplices,
                                 const float *__restrict__ cochain1_coeffs,
                                 int cochain1_size, int cochain1_dim,

                                 const int *__restrict__ cochain2_simplices,
                                 const float *__restrict__ cochain2_coeffs,
                                 int cochain2_size, int cochain2_dim,

                                 const int *__restrict__ coboundary_matrix,
                                 int matrix_stride, int active_simplices,

                                 int *__restrict__ result_simplices,
                                 float *__restrict__ result_coeffs,
                                 int *__restrict__ result_size,
                                 int max_result_size) {
  ...
  if (cochain1_dim < 0 || cochain2_dim < 0 ||
      cochain1_dim + cochain2_dim > MAX_COHOM_DIM ||
      coboundary_matrix == nullptr || matrix_stride <= 0 ||
      active_simplices <= 0) {
    return;
  }
  ...
  for (int candidate = 0; candidate < active_simplices; ++candidate) {
    const bool contains_first =
        candidate == s1 ||
        coboundary_matrix[candidate * matrix_stride + s1] != 0;
    const bool contains_second =
        candidate == s2 ||
        coboundary_matrix[candidate * matrix_stride + s2] != 0;
 ...
 */
__global__ void cohomologyReductionKernel(int *__restrict__ cocycle_matrix,
                                          int num_cocycles, int matrix_stride,
                                          int num_simplices,
