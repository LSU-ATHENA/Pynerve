...
ZigzagBenchmark benchmarkZigzag(int num_steps, int simplices_per_step) {
  ...
  for (int t = 0; t < num_steps; ++t) {
    ZigzagStep step;
    step.type = (t % 2 == 0) ? ZigzagStep::FORWARD_INCLUSION
                             : ZigzagStep::FORWARD_DELETION;
  ...
  bench.total_persistence_pairs =
      std::max<int>(bench.total_persistence_pairs, cpu_pairs_closed);
