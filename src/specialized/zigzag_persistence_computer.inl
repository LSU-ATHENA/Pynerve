...
public:
  GPUZigzagPersistence(int max_simplices, int max_steps)
      : max_simplices_(max_simplices), max_steps_(max_steps) {
    ...
    try {
      ...
      const size_t boundary_count = checkedMulSize(simplex_count, simplex_count,
                                                   "zigzag boundary matrix count");
      ...
      allocateDevice(&d_complex_simplices_, simplex_count, "allocate zigzag complex simplices");
      ...
      checkCuda(cudaMemset(d_complex_size_, 0, sizeof(int)), "initialize zigzag complex size");
    } catch (...) {
      cleanup();
      throw;
    }
  ...
  computeZigzag(const std::vector<std::vector<int>> &simplices,
                ...
                const std::vector<ZigzagStep> &steps) {
    ...
    zigzagForwardKernel<<<blocks, ZIGZAG_BLOCK_SIZE>>>(
        ...
        d_birth_indices_, d_death_indices_, d_birth_times_, d_death_times_,
        d_persistence_dim_, d_pair_count_, max_simplices_);
    ...
    for (int i = 0; i < pair_count; ++i) {
      if (h_death_idx[static_cast<size_t>(i)] >= 0) {
        float persistence = h_death_time[static_cast<size_t>(i)] -
                            h_birth_time[static_cast<size_t>(i)];
  ...
  void updateComplex(const std::vector<int> &new_simplices,
    ...
    const int num_removed = checkedIntSize(removed_simplices.size(), "zigzag removed simplex count");
    ...
    auto free_runtime = [&]() {
      if (d_new) cudaFree(d_new);
      if (d_removed) cudaFree(d_removed);
    };

    try {
      allocateDevice(&d_new, new_simplices.size(), "allocate zigzag new simplices");
      allocateDevice(&d_removed, removed_simplices.size(), "allocate zigzag removed simplices");
  ...
  computeTimeSeriesTopology(
      const std::vector<std::vector<std::vector<float>>> &time_slices,
      float max_distance) {
    ...
    auto build_snapshot =
        [max_distance](const std::vector<std::vector<float>> &points) {
    ...
    for (size_t t = 0; t < time_slices.size(); ++t) {
      ...
      for (const auto &simplex : universe) {
        ...
        dimensions.push_back(checkedIntSize(simplex.size(), "zigzag simplex dimension") - 1);
      }

      std::vector<ZigzagStep> steps;
      for (const auto &simplex : current) {
        if (!previous.contains(simplex)) {
          steps.push_back({ZigzagStep::FORWARD_INCLUSION,
                           simplex_to_index[simplex], static_cast<float>(t)});
        }
      }
      for (const auto &simplex : previous) {
        if (!current.contains(simplex)) {
          steps.push_back({ZigzagStep::FORWARD_DELETION,
                           simplex_to_index[simplex], static_cast<float>(t)});
        }
      }
...
private:
  ...
  void cleanup() noexcept {
    if (d_boundary_matrix_) cudaFree(d_boundary_matrix_);
    if (d_simplex_dims_) cudaFree(d_simplex_dims_);
    if (d_steps_) cudaFree(d_steps_);
    if (d_complex_simplices_) cudaFree(d_complex_simplices_);
    if (d_complex_size_) cudaFree(d_complex_size_);
    if (d_birth_indices_) cudaFree(d_birth_indices_);
    if (d_death_indices_) cudaFree(d_death_indices_);
    if (d_birth_times_) cudaFree(d_birth_times_);
    if (d_death_times_) cudaFree(d_death_times_);
    if (d_persistence_dim_) cudaFree(d_persistence_dim_);
