...
public:
  GPUCohomologyComputer(int max_simplices, int max_dim)
      : max_simplices_(max_simplices), max_dim_(max_dim) {
    ...
    try {
      ...
      const size_t matrix_count = checkedMulSize(simplex_count, simplex_count,
                                                 "cup product coboundary matrix count");
   ...
   */
  std::vector<Cochain>
  computeCohomologyBasis(const std::vector<std::vector<int>> &simplices,
  ...
  Cochain cupProduct(const Cochain &alpha, const Cochain &beta) {
    const int alpha_index_count = checkedIntSize(alpha.simplex_indices.size(),
                                                "alpha simplex index count");
    const int alpha_coeff_count = checkedIntSize(alpha.coefficients.size(),
                                                "alpha coefficient count");
    const int beta_index_count = checkedIntSize(beta.simplex_indices.size(),
                                               "beta simplex index count");
    const int beta_coeff_count = checkedIntSize(beta.coefficients.size(),
                                               "beta coefficient count");
    const int alpha_size = std::min({alpha.num_simplices, alpha_index_count, alpha_coeff_count});
    ...
    const size_t max_result_size = checkedMulSize(static_cast<size_t>(alpha_size),
                                                  static_cast<size_t>(beta_size),
    ...
    auto free_runtime = [&]() {
      if (d_alpha_simplices) cudaFree(d_alpha_simplices);
      if (d_alpha_coeffs) cudaFree(d_alpha_coeffs);
      if (d_beta_simplices) cudaFree(d_beta_simplices);
      if (d_beta_coeffs) cudaFree(d_beta_coeffs);
      if (d_result_simplices) cudaFree(d_result_simplices);
      if (d_result_coeffs) cudaFree(d_result_coeffs);
      if (d_result_size) cudaFree(d_result_size);
    };
    ...
    try {
      ...
      copyToDevice(d_beta_simplices, beta.simplex_indices.data(), static_cast<size_t>(beta_size),
                   "copy beta simplices");
      ...
      cupProductKernel<<<blocks, CUP_PRODUCT_BLOCK_SIZE>>>(
          d_alpha_simplices, d_alpha_coeffs, alpha_size, alpha.dimension,
          d_beta_simplices, d_beta_coeffs, beta_size, beta.dimension,
          d_coboundary_matrix_, max_simplices_, active_simplices_,
          d_result_simplices, d_result_coeffs, d_result_size, max_result);
      ...
      copyToHost(result.coefficients.data(), d_result_coeffs, static_cast<size_t>(result_size),
                 "copy cup result coefficients");
...
private:
  void buildCoboundaryMatrix(const std::vector<std::vector<int>> &simplices,
    ...
    const size_t matrix_count = checkedMulSize(static_cast<size_t>(max_simplices_),
                                               static_cast<size_t>(max_simplices_),
    ...
    try {
      ...
      for (int i = 0; i < n; ++i) {
        if (h_basis[static_cast<size_t>(i)] >= 0) {
          Cochain c;
          c.dimension = (n > 0) ? static_cast<int>((static_cast<float>(i) / n) * max_dim_) : 0;
          c.num_simplices = 1;
          c.simplex_indices = {i};
          c.coefficients = {1.0f};
          basis.push_back(c);
  ...
  void cleanup() noexcept {
    if (d_cocycle_buffer_) cudaFree(d_cocycle_buffer_);
    if (d_coboundary_matrix_) cudaFree(d_coboundary_matrix_);
    if (d_cup_product_buffer_) cudaFree(d_cup_product_buffer_);
    if (d_temp_simplices_) cudaFree(d_temp_simplices_);
    if (d_temp_coeffs_) cudaFree(d_temp_coeffs_);
