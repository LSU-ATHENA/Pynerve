...
StochasticPersistenceGradient::StochasticPersistenceGradient(
    int batch_size,
    double)
    : batch_size_(batch_size), rng_(42) {}
...
MatrixXd TopologyOptimizer::optimize(
    ...
    for (int iter = 0; iter < config_.max_iterations; ++iter) {
        ...
        MatrixXd grad = grad_computer.computeTopologyOptimizationGradient(
            points, current_diagram, target_diagram
        );
...
DifferentiableDiagram computeDifferentiable(
    std::span<const double> points_data,
    size_t n_points,
    size_t point_dim,
    double max_distance,
    int max_dim) {
...
std::vector<double> persistenceBackward(
    std::span<const double> points_data,
    size_t n_points,
    size_t point_dim,
