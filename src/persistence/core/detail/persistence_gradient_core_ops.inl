MatrixXd PersistenceGradient::computeGradient(
    ...
    for (size_t i = 0; i < diagram.persistence_pairs.size() && i < loss_gradients.size(); ++i) {
        ...
        if (pair.second == std::numeric_limits<double>::infinity()) continue;
...
MatrixXd PersistenceGradient::computeSimplexGradient(
    const MatrixXd& points,
    ...
    for (int i = 0; i < n_verts; ++i) {
        ...
        for (int j = i + 1; j < n_verts; ++j) {
            ...
            if (dist <= GRADIENT_EPSILON ||
                std::abs(dist - filtration_value) >= GRADIENT_EPSILON) {
                continue;
            }
...
MatrixXd PersistenceGradient::computeTopologyOptimizationGradient(
    ...
    validateDifferentiableDiagram(current_diagram,
                                  "PersistenceGradient::computeTopologyOptimizationGradient",
                                  true);
    ...
    for (size_t i = 0; i < current_diagram.persistence_pairs.size(); ++i) {
        ...
        for (const auto& match : matches) {
            if (match.first == static_cast<int>(i)) {
                ...
                if (target_idx >= 0 && target_idx < static_cast<int>(target_diagram.persistence_pairs.size())) {
...
std::vector<std::pair<int, int>> PersistenceGradient::matchPersistencePairs(
    const DifferentiableDiagram& current,
    ...
    for (size_t i = 0; i < current.persistence_pairs.size(); ++i) {
        ...
        for (size_t j = 0; j < target.persistence_pairs.size(); ++j) {
            if (target_matched[j]) continue;
            if (current.dimensions[i] != target.dimensions[j]) continue;

            double diff = std::abs(current.persistence_pairs[i].first -
                                   target.persistence_pairs[j].first);
            if (diff < min_diff) {
                min_diff = diff;
...
std::vector<std::pair<int, int>> PersistenceGradient::hungarianAlgorithm(
