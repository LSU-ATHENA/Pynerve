...
[[nodiscard]] size_t saturatingSize(long double value)
{
    ...
    const long double limit = static_cast<long double>(std::numeric_limits<size_t>::max());
    return value >= limit ? std::numeric_limits<size_t>::max() : static_cast<size_t>(value);
}
...
MemoryTier AutomaticMemoryOptimizer::selectTier(size_t n_points, size_t point_dim,
...
size_t AutomaticMemoryOptimizer::estimateMemoryBytes(size_t n_points, size_t point_dim,
                                                     double max_distance) const
{
    if (n_points == 0 || point_dim == 0 || !std::isfinite(max_distance) || max_distance < 0.0)
    {
        return 0;
    }
    ...
    const long double radius_density = radius <= 0.0L ? 0.0L : radius / (radius + std::sqrt(dim));
    ...
    const long double bytes_per_simplex = static_cast<long double>(sizeof(int)) * (dim + 2.0L) +
                                          static_cast<long double>(sizeof(double)) * 2.0L;

    return saturatingSize(estimated_simplices * bytes_per_simplex);
}

PersistenceDiagram AutomaticMemoryOptimizer::compute(std::span<const double> points,
                                                     size_t n_points, size_t point_dim,
                                                     double max_distance, int max_dim)
{
    PersistenceDiagram result;
    if (max_dim < 0 || point_dim == 0 || n_points == 0 || n_points > points.size() / point_dim ||
...
PersistenceDiagram computeExtremeMemory(std::span<const double> points, size_t n_points,
