...
std::vector<MorsePairing> buildMorseVectorField(const std::vector<MorseSimplex> &simplices,
...
void validateDistilledInputs(const std::vector<double> &points, size_t point_dim, size_t num_points,
                             const DistilledVRConfig &config)
{
    if (point_dim == 0)
    {
...
std::vector<MorseSimplex> buildReducedVRComplex(const std::vector<double> &points, size_t point_dim,
        ...
        for (size_t idx_a = 0; idx_a < lower_simplices.size(); ++idx_a)
        {
            for (size_t idx_b = idx_a + 1; idx_b < lower_simplices.size(); ++idx_b)
            {
                ...
                std::set_intersection(a.vertices.begin(), a.vertices.end(), b.vertices.begin(),
                                      b.vertices.end(), std::back_inserter(common));
                                             ...
                                             size_t num_points, const DistilledVRConfig &config)
{
    ...
    result.morse_time_ms =
        std::chrono::duration<double, std::milli>(end_morse - start_morse).count();
            ...
            while (!col.empty())
            {
                ...
                if (it != pivot_to_column.end())
                {
                    ...
                    std::set_symmetric_difference(
                        ...
                        boundary_matrix[it->second].end(), std::back_inserter(result_col));
                ...
                else
                {
                    ...
                    pair.birth_time =
                        filtration.simplices[static_cast<size_t>(pivot)].filtration_value;
...
DistilledSpeedupEstimate estimateDistilledSpeedup(size_t num_points, int max_dim, double max_radius)
{
    ...
    const double size_factor =
        std::clamp(std::log2(static_cast<double>(num_points) + 1.0) / 12.0, 0.75, 1.6);
    ...
    estimate.bit_parallel_speedup =
        num_points > 1000
            ? std::clamp(std::sqrt(static_cast<double>(num_points) / 1000.0), 1.0, 16.0)
            : 1.0;
