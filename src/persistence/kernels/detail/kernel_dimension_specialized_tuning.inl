                                          ...
                                          const std::vector<int> &dimensions, int max_dim)
{
    ...
    if (simplices.empty() || filtration_values.size() != simplices.size() ||
        dimensions.size() != simplices.size() || max_dim < 0)
    {
        return bench;
    }
    ...
    auto result =
        computeDimensionSpecialized(simplices, filtration_values, dimensions, max_dim, config);
    auto end = std::chrono::high_resolution_clock::now();
    bench.specialized_time_ms = std::chrono::duration<double, std::milli>(end - start).count();
