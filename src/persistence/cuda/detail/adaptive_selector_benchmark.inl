SelectorBenchmark benchmarkSelector(int num_pairs, int num_algorithms) {
  if (num_pairs < 0 || num_algorithms <= 0) {
    throw std::invalid_argument("selector benchmark sizes must be non-negative with algorithms");
  }
  ...
  bench.gpu_feature_ms =
      std::chrono::duration<double, std::milli>(end_feature - start_feature)
          .count();
  ...
  bench.gpu_predict_ms =
      std::chrono::duration<double, std::milli>(end_predict - start_predict)
          .count();
