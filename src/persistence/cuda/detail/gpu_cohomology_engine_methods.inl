static errors::ErrorResult<void> detectBirthSimplices(
...
detectEmergentPairs(const algebra::BoundaryMatrix &boundary_matrix,
...
private:
enum class BoundaryDetectionMode { Birth, Emergent };
...
detectBoundaryColumnsGpu(const algebra::BoundaryMatrix &boundary_matrix,
  ...
  try {
    ...
    if (mode == BoundaryDetectionMode::Birth) {
      ...
      launchDetectEmergentPairs(gpu_matrix.data, gpu_matrix.indices, gpu_matrix.starts, d_flags,
                                d_count, n_cols, gpu_matrix.max_height, stream);
    }
...
static void computeBirthSimplicesCpu(
    const algebra::BoundaryMatrix &boundary_matrix,
    std::vector<bool> &out_cleared, size_t &out_accelerated_ops) {
  const size_t cols = boundary_matrix.cols();
...
static void computeEmergentPairsCpu(
    const algebra::BoundaryMatrix &boundary_matrix,
...
static errors::ErrorResult<void> buildCoboundaryMatrixGpu(
    const algebra::BoundaryMatrix &boundary_matrix,
