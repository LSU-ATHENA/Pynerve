...
__device__ bool isFaceOf(const int *va, int na, const int *vb, int nb) {
    int ai = 0, bi = 0;
    while (ai < na && bi < nb) {
        if (va[ai] == vb[bi]) { ++ai; ++bi; }
        else if (va[ai] > vb[bi]) { ++bi; }
        else { return false; }
    }
    return ai == na;
}
...
__device__ int symmetricDifference(const int *a, int a_size,
                                   const int *b, int b_size,
                                   int *out, int out_capacity) {
    int oi = 0, ai = 0, bi = 0;
    while (ai < a_size && bi < b_size && oi < out_capacity) {
        if (a[ai] < b[bi])      { out[oi++] = a[ai++]; }
        else if (a[ai] > b[bi]) { out[oi++] = b[bi++]; }
        else                    { ++ai; ++bi; }        // Z2 cancel
    }
    while (ai < a_size && oi < out_capacity) { out[oi++] = a[ai++]; }
    while (bi < b_size && oi < out_capacity) { out[oi++] = b[bi++]; }
    return oi;
}
...
computeCoboundaryKernel(const SimplexGPU *__restrict__ simplices, int num_simplices,
                        int *__restrict__ cow_buffer,
                        int *__restrict__ cow_sizes,
                        int *__restrict__ cow_offsets,
                        int max_cow_size) {
    ...
    for (int j = idx + 1; j < num_simplices && count < max_cow_size; ++j) {
        ...
        if (t.dimension > target_dim) break;
        if (t.dimension < target_dim) continue;
    ...
    for (int j = idx - 1; j >= 0 && count < max_cow_size; --j) {
        ...
        if (t.dimension > target_dim) continue;
        if (t.dimension < target_dim) break;
        if (t.filtration_value != s.filtration_value) break;
        if (isFaceOf(s.vertices, s.num_vertices, t.vertices, t.num_vertices))
            cow_buffer[base + count++] = j;
    }
...
clearingKernel(SimplexGPU *__restrict__ simplices, int num_simplices,
               const int *__restrict__ pivot_table,
               int *__restrict__ cow_sizes, int current_dim) {
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx >= num_simplices) return;

    SimplexGPU s = simplices[idx];
    if (s.dimension != current_dim) return;
...
__global__ void __launch_bounds__(256)
coboundaryReductionKernel(int *__restrict__ column_data,
                          int *__restrict__ column_sizes,
                          int *__restrict__ pivot_table,
                          int num_columns, int max_cow_size,
                          int num_simplices,
                          const SimplexGPU *__restrict__ simplices,
                          int current_dim) {
    int col_idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (col_idx >= num_columns) return;

    // In dimension-cascade mode, only process columns of this dimension
    if (current_dim >= 0 && simplices[col_idx].dimension != current_dim) return;
    ...
    for (int iter = 0; iter < MAX_ITER && local_size > 0; ++iter) {
        ...
        if (pivot < 0 || pivot >= num_simplices) break;
        ...
        for (int i = 0; i < local_size; ++i) dst[i] = col[i];
        __threadfence();
        ...
        if (other_size <= 0) break;

        int tmp[SCRATCH];
        int ns = symmetricDifference(col, local_size,
                                     other, other_size,
                                     tmp, SCRATCH);
        for (int i = 0; i < ns && i < SCRATCH; ++i) col[i] = tmp[i];
        local_size = ns;
    }
}
...
apparentPairKernel(const SimplexGPU *__restrict__ simplices, int num_simplices,
                   const int *__restrict__ cow_sizes,
                   const int *__restrict__ cow_offsets,
                   const int *__restrict__ cow_buffer,
                   int *__restrict__ pivot_table,
                   int max_cow_size) {
    int sigma_idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (sigma_idx >= num_simplices) return;

    const SimplexGPU sigma = simplices[sigma_idx];
    const int d = sigma.dimension;
    if (d < 0 || d + 1 > MAX_DIM_GPU_COHOMOLOGY) return;

    int base = cow_offsets[sigma_idx];
    int nc = cow_sizes[sigma_idx];
    if (nc <= 0 || nc > max_cow_size) return;
    ...
    for (int i = 1; i < nc; ++i) {
        ...
        if (cf > youngest_filt) {
            youngest_idx = ci; youngest_filt = cf;
        }
    ...
    for (int other_idx = 0; other_idx < num_simplices; ++other_idx) {
        if (other_idx == sigma_idx) continue;
        const SimplexGPU other = simplices[other_idx];
        if (other.dimension != d) continue;
        if (!isFaceOf(other.vertices, other.num_vertices,
                      tau.vertices, tau.num_vertices))
            continue;
        if (other.filtration_value < sigma.filtration_value ||
            (other.filtration_value == sigma.filtration_value &&
             other.index < sigma.index)) {
...
static void extractPairs(const SimplexGPU *simplices, int num_simplices,
                         const int *pivot_table,
                         const int *original_sizes,
                         std::vector<Pair> &out_pairs) {
    out_pairs.clear();
    ...
    for (int p = 0; p < num_simplices; ++p) {
        ...
        if (pair.death <= pair.birth) continue;
    ...
    for (int i = 0; i < num_simplices; ++i) {
        if (original_sizes[i] <= 0) continue;
        if (is_birth[i]) continue;
        if (is_death[i]) continue;
...
GPUCohomologyComputer::GPUCohomologyComputer()
    : initialized_(false), last_compute_time_ms_(0.0), cpu_baseline_ms_(0.0) {}
...
bool GPUCohomologyComputer::initialize(size_t max_simplices, size_t max_dim) {
    ...
    try {
        ...
        initialized_ = true;
        return true;
    } catch (...) { return initialized_ = false; }
}

bool GPUCohomologyComputer::computePersistenceDiagram(
    std::span<const SimplexGPU> simplices, std::vector<Pair> &persistence_pairs) {
    if (!initialized_) return false;
    int N = static_cast<int>(simplices.size());
    if (N == 0) return false;
    if (static_cast<size_t>(N) > d_simplices_.size()) return false;
    ...
    computeCoboundaryKernel<<<GS, BS>>>(
        thrust::raw_pointer_cast(d_simplices_.data()), N,
        ...
        COBOUNDARY_MAX_SIZE);
    if (cudaGetLastError() != cudaSuccess) { cudaEventDestroy(start); cudaEventDestroy(stop); return false; }
    ...
    for (const auto &s : simplices) {
        if (s.dimension > max_dim) max_dim = s.dimension;
        if (max_dim >= MAX_DIM_GPU_COHOMOLOGY) break;
    }
    ...
    for (int d = 0; d <= max_dim; ++d) {
        ...
        coboundaryReductionKernel<<<GS, BS>>>(
            thrust::raw_pointer_cast(d_red_data.data()),
            thrust::raw_pointer_cast(d_red_sizes_.data()),
            thrust::raw_pointer_cast(d_pivot_table_.data()),
            N, COBOUNDARY_MAX_SIZE, N,
    ...
    if (cudaMemcpy(h_orig_sizes.data(),
                   thrust::raw_pointer_cast(d_coboundary_sizes_.data()),
                   static_cast<size_t>(N) * sizeof(int),
                   cudaMemcpyDeviceToHost) != cudaSuccess) {
        cudaEventDestroy(start); cudaEventDestroy(stop);
    ...
    extractPairs(h_simplices.data(), N, h_pivot.data(),
                 h_orig_sizes.data(), persistence_pairs);
...
double GPUCohomologyComputer::getLastComputeTime() const noexcept { return last_compute_time_ms_; }

double GPUCohomologyComputer::getSpeedupVsCPU() const noexcept {
    if (cpu_baseline_ms_ <= 0.0 || last_compute_time_ms_ <= 0.0) return 0.0;
    return cpu_baseline_ms_ / last_compute_time_ms_;
}

void GPUCohomologyComputer::setCPUBaseline(double cpu_ms) noexcept { cpu_baseline_ms_ = cpu_ms; }
...
bool GPUCohomologyComputer::getReducedColumnSizes(std::vector<int> &out_sizes) const {
    ...
    out_sizes.resize(N);
    cudaError_t err = cudaMemcpy(out_sizes.data(),
                                  thrust::raw_pointer_cast(d_red_sizes_.data()),
                                  N * sizeof(int),
...
bool GPUCohomologyComputer::reduceCoboundaries(int num_simplices) {
    ...
    coboundaryReductionKernel<<<GS, BS>>>(
        ...
        thrust::raw_pointer_cast(d_coboundary_sizes_.data()),
        thrust::raw_pointer_cast(d_pivot_table_.data()),
        num_simplices, COBOUNDARY_MAX_SIZE, num_simplices,
        thrust::raw_pointer_cast(d_simplices_.data()), -1);   // -1 = process all dims
    return cudaGetLastError() == cudaSuccess;
}

/* High-level API: run full GPU cohomology pipeline on SimplexGPU data */
PersistenceDiagram computeGPUCohomology(const std::vector<SimplexGPU> &simplices,
                                        int max_dimension) {
