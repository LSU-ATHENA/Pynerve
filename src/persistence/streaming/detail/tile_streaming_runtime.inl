...
{
public:
    MemoryMappedFile(const std::string &path, size_t offset_bytes, size_t size_bytes)
        : path_(path)
        ...
        , size_bytes_(size_bytes)
    {
        ...
        mapped_data_ = mmap(nullptr, mapped_size_bytes_, PROT_READ, MAP_PRIVATE, fd_,
    ...
    {
        if (point_dim != 0 && count > std::numeric_limits<size_t>::max() / point_dim)
        {
        ...
        if (point_dim != 0 && start_idx > std::numeric_limits<size_t>::max() / point_dim)
        {
...
#if defined(__linux__)
        if (mapped_data_ != nullptr && chunk_offset_bytes + chunk_size_bytes <= size_bytes_)
        {
            const auto *src =
                static_cast<const char *>(mapped_data_) + map_shift_bytes_ + chunk_offset_bytes;
        ...
        const auto seek_to = static_cast<std::streamoff>(offset_bytes_ + chunk_offset_bytes);
        ...
        if (!file)
        {
            throw std::runtime_error("Failed to read expected chunk from input file: " + path_);
        }
...
public:
    ...
    [[nodiscard]] static std::vector<Tile> partitionLinear(size_t num_points, size_t max_tile_size,
                                                           double overlap_ratio)
    {
        ...
        {
            throw std::invalid_argument("tile overlap_ratio must be finite and in [0, 0.5]");
        }
        const size_t overlap_points =
            std::min(max_tile_size > 1 ? max_tile_size - 1 : static_cast<size_t>(0),
                     static_cast<size_t>(static_cast<double>(max_tile_size) * overlap_ratio));

        for (size_t core_start = 0; core_start < num_points;)
        {
            const size_t core_count = std::min(max_tile_size, num_points - core_start);
            const size_t core_end = core_start + core_count;
            const size_t read_start =
                (core_start > overlap_points) ? (core_start - overlap_points) : 0;
            const size_t read_end = core_end + std::min(overlap_points, num_points - core_end);
    ...
    [[nodiscard]] static std::vector<Tile> partitionPoints(const std::vector<double> &points,
                                                           ...
                                                           double overlap_ratio = 0.1)
    {
        ...
        for (auto &tile : tiles)
        {
            tile.bounds = computeBounds(points, point_dim, tile.start_idx, tile.local_count);
        }
        return tiles;
    }
...
private:
    ...
    computeBounds(const std::vector<double> &points, size_t point_dim, size_t start, size_t count)
    {
...
{
public:
    explicit AsyncTileProcessor(size_t num_threads)
        : pool_(num_threads)
    {}

    [[nodiscard]] std::future<std::vector<Pair>> submitTile(const std::vector<double> &tile_points,
                                                            size_t point_dim,
                                                            const FloodComplexConfig &config)
    {
        return pool_.enqueue([tile_points, point_dim, config]() {
            const size_t num_points = point_dim == 0 ? 0 : (tile_points.size() / point_dim);
            auto result = computeFloodComplex(tile_points, point_dim, num_points, config);
            return result.pairs;
        });
    }
...
public:
                 ...
                 double merge_tolerance = kDefaultTileMergeTolerance)
    {
        ...
        {
            if (reserve_pairs > std::numeric_limits<size_t>::max() - tile_pairs.size())
            {
        ...
        const double tolerance = std::max(merge_tolerance, std::numeric_limits<double>::epsilon());
