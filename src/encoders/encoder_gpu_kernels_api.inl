...
public:
    GPUTopologicalEncoder(int topo_features, const std::vector<int> &mlp_layers)
        : topo_features_(topo_features)
        , mlp_layers_(mlp_layers)
    {
        if (topo_features_ <= 0 || mlp_layers_.size() < 2 || mlp_layers_.front() != topo_features_)
        {
        ...
        try
        {
            for (size_t i = 0; i + 1 < mlp_layers_.size(); ++i)
            {
                ...
                const size_t weight_count =
                    checkedMulSize(static_cast<size_t>(in_dim), static_cast<size_t>(out_dim),
                                   "encoder layer weight count");
                float *d_weights = nullptr;
                float *d_bias = nullptr;
                allocateDevice(&d_weights, weight_count, "allocate encoder layer weights");
                allocateDevice(&d_bias, static_cast<size_t>(out_dim),
                               "allocate encoder layer bias");
                ...
                for (int out_idx = 0; out_idx < out_dim; ++out_idx)
                {
                    bias[static_cast<size_t>(out_idx)] =
                        deterministicEncoderBias(static_cast<int>(i), out_idx);
                }
                copyToDevice(d_weights, weights.data(), weights.size(),
                             "copy encoder layer weights");
    ...
    void encode(const std::vector<float> &diagrams, std::vector<float> &output, int batch_size,
                int max_pairs)
    {
        ...
        try
        {
            ...
            const size_t workspace_count =
                checkedMulSize(static_cast<size_t>(batch_size), static_cast<size_t>(workspace_dim),
            ...
            validateDeviceFinite(d_features,
                                 checkedMulSize(static_cast<size_t>(batch_size),
                                                static_cast<size_t>(topo_features_),
            ...
            for (size_t i = 0; i + 1 < mlp_layers_.size(); ++i)
            {
                ...
                fusedMLPLayerKernel<<<grid2, block2>>>(layer_input, d_mlp_weights_[i],
                                                       d_mlp_bias_[i], layer_output, batch_size,
                                                       in_dim, out_dim, 0);
                checkCuda(cudaPeekAtLastError(), "launch encoder MLP layer");
                checkCuda(cudaDeviceSynchronize(), "synchronize encoder MLP layer");
                validateDeviceFinite(layer_output,
                                     checkedMulSize(static_cast<size_t>(batch_size),
    ...
    void encodeFused(const std::vector<float> &diagrams, std::vector<float> &output, int batch_size,
                     int max_pairs)
    {
        validateEncodeInput(diagrams, batch_size, max_pairs);
        ...
        try
        {
            ...
            copyToDevice(d_diagrams, diagrams.data(), diagrams.size(),
                         "copy fused encoder diagrams");

            const size_t smem_size =
                checkedMulSize(static_cast<size_t>(topo_features_), sizeof(float),
                               "fused encoder shared memory bytes");
            fusedTopologicalEncoderKernel<<<batch_size, BLOCK_SIZE, smem_size>>>(
                d_diagrams, d_mlp_weights_[0], d_mlp_bias_[0], d_output, batch_size, max_pairs,
                topo_features_, mlp_layers_.back());
...
private:
                             ...
                             int max_pairs) const
    {
        ...
        const size_t expected = checkedMulSize(checkedMulSize(static_cast<size_t>(batch_size),
        ...
        if (diagrams.size() != expected)
        {
            throw std::invalid_argument(
                "diagram buffer shape does not match batch and pair counts");
        }
...
public:
    ...
    void prefetchToGPU()
    {
        ...
        checkCuda(cudaMemPrefetchAsync(
                      buffer_,
                      checkedMulSize(buffer_size_, sizeof(float), "unified encoder prefetch bytes"),
                      location, 0u, nullptr),
