std::vector<float> computeCpuTopologicalFeatures(const std::vector<float> &diagrams, int batch_size,
                                                 int max_pairs, int num_features)
{
        ...
        for (int feature_idx = 0; feature_idx < num_features; ++feature_idx)
        {
            ...
            switch (feature_idx % 8)
            {
                ...
                case 5:
                {
                    ...
                    for (int i = 0; i < max_pairs; ++i)
                    {
                        const float p = (diagram[2 * i + 1] - diagram[2 * i]) / (total + 1e-8f);
                        if (p > 0.0f)
                            value -= p * std::log(p);
                    }
                    break;
...
std::vector<float> cpuDenseEncoder(std::vector<float> layer_input, const std::vector<int> &layers,
                                   int batch_size)
{
            ...
            for (int out_idx = 0; out_idx < out_dim; ++out_idx)
            {
                ...
                for (int in_idx = 0; in_idx < in_dim; ++in_idx)
                {
                    sum += layer_input[batch * in_dim + in_idx] *
                           deterministicEncoderWeight(static_cast<int>(layer), in_idx, out_idx);
                }
...
EncoderGPUBenchmark benchmarkGPUEncoder(int batch_size, int num_features)
{
    ...
    for (int sample = 0; sample < batch_size; ++sample)
    {
        for (int pair = 0; pair < max_pairs; ++pair)
        {
            ...
            const float birth = static_cast<float>((sample * 31 + pair * 17) % 101) / 101.0f;
            diagrams[offset] = birth;
            diagrams[offset + 1] = birth + 0.02f + static_cast<float>((pair * 13) % 37) / 370.0f;
        }
    }
    ...
    auto cpu_features =
        computeCpuTopologicalFeatures(diagrams, batch_size, max_pairs, layers.front());
    auto cpu_output = cpuDenseEncoder(std::move(cpu_features), layers, batch_size);
    ...
    bench.speedup_gpu = finiteBenchmarkSpeedup(bench.cpu_time_ms, bench.gpu_time_ms);
    bench.speedup_fused = finiteBenchmarkSpeedup(bench.cpu_time_ms, bench.fused_time_ms);
