__device__ bool finiteLifetime(const float *__restrict__ diagram, int pair_idx,
                               float &lifetime) {
...
__device__ float computeTopologicalFeatureValue(const float *__restrict__ diagram,
                                                int max_pairs,
                                                int feature_idx) {
...
__global__ void __launch_bounds__(128) topologicalFeatureExtractionKernel(
    const float *__restrict__ persistence_diagrams,
    float *__restrict__ features,
    int batch_size, int max_pairs_per_sample, int num_features) {
  ...
  const float *diagram =
      persistence_diagrams + batch_idx * max_pairs_per_sample * 2;
...
__global__ void __launch_bounds__(256) fusedMLPLayerKernel(const float *__restrict__ input,
...
__global__ void __launch_bounds__(256) fusedTopologicalEncoderKernel(
    const float *__restrict__ diagrams, const float *__restrict__ mlp_weights,
    const float *__restrict__ mlp_bias, float *__restrict__ output,
    int batch_size, int max_pairs, int topo_features, int out_features) {
  ...
  for (int feature_idx = tid; feature_idx < topo_features; feature_idx += blockDim.x) {
    shared_topo[feature_idx] =
        computeTopologicalFeatureValue(diagram, max_pairs, feature_idx);
  }
  __syncthreads();
