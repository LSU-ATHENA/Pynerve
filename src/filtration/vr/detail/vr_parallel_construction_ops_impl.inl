...
public:
    explicit LockFreeEdgeList(size_t initial_capacity = 1024)
        : edges_(initial_capacity), count_{0} {}
...
std::vector<Edge> parallelEdgeDetection(
    ...
    const size_t max_edges =
        n > std::numeric_limits<size_t>::max() / 20 ? std::numeric_limits<size_t>::max()
                                                    : n * 20;
...
VRComplex buildParallelVRComplex(
    const std::vector<Point>& points,
    float max_distance,
    int max_dim
) {
    ...
    if (max_dim >= 2) {
        ...
        #endif
        for (size_t i = 0; i < adjacency.bucket_count(); ++i) {
        ...
        #endif
        for (size_t i = 0; i < complex.edges.size(); ++i) {
...
VRBenchmark benchmarkParallelVR(int n_points, float threshold, int num_threads) {
    ...
    bench.sequential_time_ms = std::chrono::duration<double, std::milli>(
        end_seq - start_seq).count();

    auto start_par = std::chrono::high_resolution_clock::now();
    auto edges_par = parallelEdgeDetection(points, threshold, num_threads);
    auto end_par = std::chrono::high_resolution_clock::now();
    bench.parallel_time_ms = std::chrono::duration<double, std::milli>(
