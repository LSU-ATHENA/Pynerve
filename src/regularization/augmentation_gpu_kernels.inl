struct RNGState
{
    ...
    __device__ void init(unsigned int seed, int idx) { curand_init(seed, idx, 0, &state); }
...
__global__ void gaussianNoiseKernel(const float *__restrict__ input, float *__restrict__ output,
                                    int num_elements, float mean, float stddev, unsigned int seed)
{
...
__global__ void uniformNoiseKernel(const float *__restrict__ input, float *__restrict__ output,
...
__global__ void geometricTransformKernel(const float *__restrict__ input,
                                         ...
                                         int feature_dim, float rotation_angle, float scale_factor)
{
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
...
__global__ void persistenceAwareNoiseKernel(const float *__restrict__ input,
...
__global__ void mixupKernel(const float *__restrict__ input1, const float *__restrict__ input2,
                            float *__restrict__ output, int num_elements, float alpha,
                            unsigned int seed)
{
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx >= num_elements)
...
__global__ void cutoutKernel(const float *__restrict__ input, float *__restrict__ output,
                             int num_samples, int feature_dim, int mask_size, float mask_value,
                             unsigned int seed)
{
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx >= num_samples)
