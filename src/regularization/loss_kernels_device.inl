...
__global__ void wassersteinLossKernel(const float *__restrict__ target_pairs,
                                      ...
                                      float *__restrict__ loss,
                                      int num_pairs_target, int num_pairs_pred,
                                      int p_norm) {
  int tid = threadIdx.x;
  int bid = blockIdx.x;
  ...
  for (int i = tid; i < num_pairs_target; i += blockDim.x) {
    ...
    if (!isfinite(pred_birth) || !isfinite(pred_death) || !isfinite(target_birth) ||
        !isfinite(target_death) || !isfinite(d_birth) || !isfinite(d_death) ||
        !isfinite(dist)) {
      dist = INFINITY;
    }
...
__global__ void
persistenceEntropyKernel(const float *__restrict__ persistence_values,
...
__global__ void topologySmoothnessKernel(const float *__restrict__ features,
                                         ...
                                         float *__restrict__ smoothness_loss) {
  int node = blockIdx.x;
...
__global__ void
combinedTopologyLossKernel(const float *__restrict__ betti_target,
                           const float *__restrict__ betti_pred,
                           const float *__restrict__ persistence_values,
                           int num_dims, int num_persistence,
                           float lambda_betti, float lambda_persistence,
