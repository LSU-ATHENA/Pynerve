...
void copyFromDevice(std::vector<T> &dst, const T *src, const char *label) {
  GPU_CHECK(cudaMemcpy(dst.data(), src, checkedBytes<T>(dst.size(), label),
...
void requireFiniteNonNegativeInput(const std::vector<float> &values,
                                   const char *label) {
...
public:
  ...
  std::vector<float> augment(const std::vector<float> &data, int num_samples,
                             int feature_dim) {
    if (num_samples <= 0 || feature_dim <= 0) {
      return {};
    }
    std::size_t total_values = 0;
    if (!checkedProduct(static_cast<std::size_t>(num_samples), static_cast<std::size_t>(feature_dim),
                        total_values)) {
      ...
      for (auto &aug_type : config_.augmentations) {
        ...
        case AugmentationType::GAUSSIAN_NOISE:
          gaussianNoiseKernel<<<blocks, BLOCK_SIZE>>>(
              d_input, d_output, total_elements, 0.0f, config_.gaussian_std,
              seed++);
        ...
        case AugmentationType::CUTOUT:
          cutoutKernel<<<sample_blocks, BLOCK_SIZE>>>(
              d_input, d_output, num_samples, feature_dim, config_.cutout_size,
              0.0f, seed++);
          GPU_CHECK(cudaPeekAtLastError());
          break;

        default:
          GPU_CHECK(cudaMemcpy(d_output, d_input, checkedBytes<float>(total_values, "augmentation copy"),
  ...
  std::vector<float>
  augmentWithPersistence(const std::vector<float> &data,
    ...
    const int n = checkedIntSize(data.size(), "persistence augmentation size exceeds CUDA limits");
    ...
    try {
      ...
      persistenceAwareNoiseKernel<<<blocks, BLOCK_SIZE>>>(
          d_input, d_persistence, d_output, n, config_.gaussian_std,
  ...
  std::vector<float> mixup(const std::vector<float> &data1,
    ...
    try {
      ...
      mixupKernel<<<blocks, BLOCK_SIZE>>>(d_in1, d_in2, d_out, n,
                                          config_.mixup_alpha, seed);
      GPU_CHECK(cudaPeekAtLastError());
...
private:
  void validateConfig() const {
    if (!std::isfinite(config_.gaussian_std) || config_.gaussian_std < 0.0f ||
        ...
        !std::isfinite(config_.rotation_range) ||
        !std::isfinite(config_.scale_range) ||
        !std::isfinite(1.0f + config_.scale_range) ||
        !std::isfinite(config_.persistence_scale) ||
        config_.persistence_scale < 0.0f ||
