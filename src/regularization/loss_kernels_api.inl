...
void copyToDevice(T *dst, const std::vector<T> &src, const char *label) {
  ...
  GPU_CHECK(cudaMemcpy(dst, src.data(), checkedBytes<T>(src.size(), label),
...
void requireFiniteDiagram(const std::vector<std::pair<float, float>> &diagram,
                          const char *label) {
  for (const auto &pair : diagram) {
    if (!std::isfinite(pair.first) || !std::isfinite(pair.second) ||
        pair.second < pair.first) {
      throw std::invalid_argument(std::string(label) + " pairs must be finite with death >= birth");
    }
  }
}
...
public:
  ...
  explicit GPUTopologyLoss(const LossConfig &config) : config_(config), d_loss_buffer_(nullptr) {
  ...
  float bettiMSELoss(const std::vector<float> &target,
  ...
  float
  wassersteinLoss(const std::vector<std::pair<float, float>> &target_diagram,
    ...
    const int n_target = checkedIntSize(target_diagram.size(), "target diagram size exceeds CUDA limits");
    const int n_pred = checkedIntSize(pred_diagram.size(), "predicted diagram size exceeds CUDA limits");
    const std::size_t target_values = checkedPairValues(target_diagram.size(), "target diagram values overflow");
    const std::size_t pred_values = checkedPairValues(pred_diagram.size(), "predicted diagram values overflow");

    std::vector<float> flat_target(target_values);
    std::vector<float> flat_pred(pred_values);

    for (int i = 0; i < n_target; ++i) {
      flat_target[static_cast<std::size_t>(i) * 2U] = target_diagram[static_cast<std::size_t>(i)].first;
      flat_target[static_cast<std::size_t>(i) * 2U + 1U] = target_diagram[static_cast<std::size_t>(i)].second;
    }

    for (int i = 0; i < n_pred; ++i) {
      flat_pred[static_cast<std::size_t>(i) * 2U] = pred_diagram[static_cast<std::size_t>(i)].first;
      flat_pred[static_cast<std::size_t>(i) * 2U + 1U] = pred_diagram[static_cast<std::size_t>(i)].second;
    }
    ...
    try {
      ...
      wassersteinLossKernel<<<n_pred, BLOCK_SIZE>>>(
          d_target, d_pred, d_loss_buffer_, n_target, n_pred, config_.p_norm);
      GPU_CHECK(cudaPeekAtLastError());
      const float loss = readDeviceLoss(d_loss_buffer_);
      cudaFree(d_target);
      cudaFree(d_pred);
  ...
  float persistenceEntropy(const std::vector<float> &persistence_values) {
    ...
    const int n = checkedIntSize(persistence_values.size(), "persistence value count exceeds CUDA limits");
  ...
  float smoothnessLoss(const std::vector<float> &features,
                       const std::vector<int> &adjacency, int num_nodes,
                       int feature_dim) {
    ...
    if (!checkedProduct(static_cast<std::size_t>(num_nodes), static_cast<std::size_t>(feature_dim),
                        feature_values) ||
        !checkedProduct(static_cast<std::size_t>(num_nodes), static_cast<std::size_t>(num_nodes),
                        adjacency_values)) {
    ...
    try {
      ...
      topologySmoothnessKernel<<<num_nodes, feature_dim>>>(
          d_features, d_adjacency, num_nodes, feature_dim, d_loss_buffer_);
      GPU_CHECK(cudaPeekAtLastError());
      const float loss = readDeviceLoss(d_loss_buffer_);
      cudaFree(d_features);
  ...
  float combinedLoss(const std::vector<float> &betti_target,
                     const std::vector<float> &betti_pred,
    ...
    requireNonnegativeFiniteValues(persistence_values, "persistence values");
    const int num_dims = checkedIntSize(betti_target.size(), "Betti dimension count exceeds CUDA limits");
