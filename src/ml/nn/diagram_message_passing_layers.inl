...
public:
  enum class AggregationType { MEAN, SUM, MAX, ATTENTION };

  GPUDiagramGNNLayer(int max_pairs, int feature_dim, AggregationType agg_type)
      : max_pairs_(max_pairs), feature_dim_(feature_dim), agg_type_(agg_type) {
    if (max_pairs_ <= 0 || feature_dim_ <= 0) {
      throw std::invalid_argument("diagram GNN dimensions must be positive");
    }
    pair_matrix_count_ = checkedMulSize(static_cast<size_t>(max_pairs_),
                                        static_cast<size_t>(max_pairs_),
                                        "diagram pair matrix count");
    feature_count_capacity_ = checkedMulSize(static_cast<size_t>(max_pairs_),
    ...
    try {
      ...
      allocateDevice(&d_attention_weights_, pair_matrix_count_, "allocate diagram attention weights");
  ...
  void computeAdjacency(const std::vector<float> &diagrams, float bandwidth) {
    ...
    try {
      ...
      copyToDevice(d_diagrams, diagrams.data(), diagrams.size(), "copy diagram adjacency input");
      ...
      diagramDistanceKernel<<<blocks, threads>>>(d_diagrams, d_adjacency_, num_pairs, bandwidth);
  ...
  std::vector<float> forward(const std::vector<float> &node_features,
    ...
    const int num_pairs = validateDiagramBuffer(diagrams);
    const size_t feature_count = checkedMulSize(static_cast<size_t>(num_pairs),
                                                static_cast<size_t>(feature_dim_),
    ...
    try {
      ...
      copyToDevice(d_features, node_features.data(), node_features.size(), "copy diagram features");
      ...
      if (agg_type_ == AggregationType::ATTENTION) {
        ...
        const size_t smem_size = checkedMulSize(
            static_cast<size_t>(num_pairs + threads_per_pair), sizeof(float),
            "diagram attention shared memory bytes");
        diagramAttentionKernel<float><<<blocks, threads, smem_size>>>(
            d_query_, d_key_, d_value_, d_diagrams, d_attention_weights_, d_messages_, num_pairs,
            feature_dim_, 1.0f);
        checkCuda(cudaPeekAtLastError(), "launch diagram attention kernel");
      } else {
        dim3 blocks(num_pairs);
        dim3 threads(std::min(feature_dim_, BLOCK_SIZE));
        const size_t smem_size = checkedMulSize(static_cast<size_t>(num_pairs), sizeof(float),
  ...
  std::vector<float> getAttentionWeights() const {
    ...
    copyToHost(weights.data(), d_attention_weights_, weights.size(), "copy diagram attention weights");
    return weights;
  }
...
private:
  ...
  void cleanup() noexcept {
    if (d_adjacency_) cudaFree(d_adjacency_);
    if (d_messages_) cudaFree(d_messages_);
    if (d_attention_weights_) cudaFree(d_attention_weights_);
...
public:
  GPUMultiScaleDiagramConv(int max_pairs, int feature_dim,
                           const std::vector<float> &scales)
      : max_pairs_(max_pairs), feature_dim_(feature_dim), scales_(scales) {
    if (max_pairs_ <= 0 || feature_dim_ <= 0) {
    ...
    try {
      ...
      allocateDevice(&d_combined_, feature_count_capacity_, "allocate multiscale combined output");
    } catch (...) {
      cleanup();
      throw;
    }
  ...
  std::vector<float> forward(const std::vector<float> &diagrams,
    ...
    const int num_pairs = validateDiagramBuffer(diagrams);
    const size_t total_values = checkedMulSize(static_cast<size_t>(num_pairs),
                                               static_cast<size_t>(feature_dim_),
                                               "multiscale diagram total values");
    if (features.size() != total_values) {
      throw std::invalid_argument("feature vector size does not match num_pairs * feature_dim");
    }
    ...
    auto free_runtime = [&]() {
      if (d_scale_messages) cudaFree(d_scale_messages);
      if (d_features) cudaFree(d_features);
      if (d_diagrams) cudaFree(d_diagrams);
    };
    ...
    try {
      ...
      checkCuda(cudaMemset(d_combined_, 0,
                           checkedMulSize(total_values, sizeof(float),
                                          "multiscale combined bytes")),
      ...
      const size_t message_smem = checkedMulSize(static_cast<size_t>(num_pairs), sizeof(float),
                                                 "multiscale message shared memory bytes");
      const int total_values_int = checkedIntSize(total_values, "multiscale total value count");
      const int reduce_blocks = static_cast<int>((total_values + BLOCK_SIZE - 1) / BLOCK_SIZE);
      ...
      for (size_t i = 0; i < scales_.size(); ++i) {
        ...
        diagramMessagePassingKernel<float><<<message_blocks, message_threads, message_smem>>>(
            d_features, adjacency_per_scale_[i], d_scale_messages, num_pairs, feature_dim_, false);
