 ...
 */
__device__ __inline__ float birthDeathDistance(float b1, float d1, float b2, float d2)
{
    float db = b1 - b2;
    float dd = d1 - d2;
    float db_sq = db * db;
    ...
    return (isfinite(db) && isfinite(dd) && isfinite(db_sq) && isfinite(dd_sq) && isfinite(sum) &&
            isfinite(distance))
               ? distance
               : INFINITY;
}
...
__global__ void diagramDistanceKernel(const float *__restrict__ diagrams,
                                      float *__restrict__ distance_matrix, int num_pairs,
                                      float bandwidth)
{
...
__global__ void diagramMessagePassingKernel(const T *__restrict__ node_features,
                                            ...
                                            T *__restrict__ messages, int num_pairs,
                                            int feature_dim, bool use_attention)
{
    int pair = blockIdx.x;
    ...
    for (int neighbor = 0; neighbor < num_pairs; ++neighbor)
    {
        ...
        if (isfinite(w) && w > EPSILON)
        {
            ...
            if (!isfinite(static_cast<float>(feature)) ||
                !isfinite(static_cast<float>(contribution)) ||
...
template <typename T>
__global__ void
diagramAttentionKernel(const T *__restrict__ query, const T *__restrict__ key,
                       const T *__restrict__ value, const float *__restrict__ diagrams,
                       float *__restrict__ attention_weights, T *__restrict__ output, int num_pairs,
                       int feature_dim, float temperature)
{
    ...
    if (tid == 0)
    {
        ...
        for (int i = 0; i < num_pairs; ++i)
        {
            ...
            s_scores[i] = (isfinite(normalized) && isfinite(exp_score) && isfinite(next))
                              ? exp_score
                              : INFINITY;
...
template <typename T>
__global__ void
diagramConvolutionKernel(const T *__restrict__ input, const float *__restrict__ kernels,
                         const float *__restrict__ diagrams, T *__restrict__ output, int num_pairs,
                         int in_channels, int out_channels, int kernel_size, float bandwidth)
{
    ...
    for (int in_ch = 0; in_ch < in_channels; ++in_ch)
    {
        for (int k = 0; k < kernel_size; ++k)
        {
            ...
            for (int other = 0; other < num_pairs; ++other)
            {
                ...
                const float scaled = -dist * dist / denom;
                float weight =
                    (isfinite(dist) && isfinite(denom) && denom > 0.0f && isfinite(scaled))
                        ? expf(scaled)
                        : INFINITY;
                ...
                if (!isfinite(kb) || !isfinite(kd) || !isfinite(b2) || !isfinite(d2) ||
                    ...
                    !isfinite(static_cast<float>(k_val)) ||
                    !isfinite(static_cast<float>(contribution)) ||
...
__global__ void accumulateScaleKernel(const float *__restrict__ scale_messages,
                                      float *__restrict__ combined, int total_values, float weight)
{
    ...
    if (idx < total_values)
    {
        ...
        combined[idx] = (isfinite(weight) && isfinite(scale_messages[idx]) &&
                         isfinite(contribution) && isfinite(next))
                            ? next
                            : INFINITY;
    }
}
