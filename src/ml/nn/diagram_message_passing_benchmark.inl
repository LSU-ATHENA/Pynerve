  ...
  for (int pair = 0; pair < num_pairs; ++pair) {
    for (int neighbor = 0; neighbor < num_pairs; ++neighbor) {
      ...
      for (int feat = 0; feat < feature_dim; ++feat) {
        dot += features[pair * feature_dim + feat] *
               features[neighbor * feature_dim + feat];
      }
      ...
      scores[neighbor] = dot * inv_sqrt_dim - std::sqrt(db * db + dd * dd) *
                                                  TOPOLOGICAL_BIAS_PENALTY;
    }
...
DiagramGNNSpeedup benchmarkDiagramGNN(int num_pairs, int feature_dim) {
  ...
  GPUDiagramGNNLayer gnn(num_pairs, feature_dim,
                         GPUDiagramGNNLayer::AggregationType::ATTENTION);

  // Deterministic diagram workload for CPU/GPU timing.
  std::vector<float> diagrams(num_pairs * DIAGRAM_FIELDS_PER_PAIR);
  for (int i = 0; i < num_pairs; ++i) {
    diagrams[i * DIAGRAM_FIELDS_PER_PAIR + 0] =
        static_cast<float>((i * 11) % 97) / 97.0f;
    diagrams[i * DIAGRAM_FIELDS_PER_PAIR + 1] =
        diagrams[i * DIAGRAM_FIELDS_PER_PAIR + 0] + 0.05f +
        static_cast<float>((i * 7) % 31) / 310.0f;
    diagrams[i * DIAGRAM_FIELDS_PER_PAIR + 2] =
        static_cast<float>(i % DIAGRAM_FIELDS_PER_PAIR);
  }
  ...
  auto cpu_output =
      cpuAttentionForward(features, diagrams, num_pairs, feature_dim, 1.0f);
  ...
  bench.speedup =
      bench.gpu_time_ms > 0.0 ? bench.cpu_time_ms / bench.gpu_time_ms : 0.0;
