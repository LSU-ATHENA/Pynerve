StreamingPHMicrobenchmark::StreamingPHMicrobenchmark(const MicrobenchmarkConfig &config)
    : config_(config)
...
MicrobenchmarkResult StreamingPHMicrobenchmark::runStreamingPhBenchmark()
{
    ...
    const auto end = t1;
    return makeResult("streaming_ph", {std::chrono::duration<double, std::micro>(t1 - t0).count()},
                      start, end);
}
...
{
    const auto points =
        generateTimeSeries(std::max<std::size_t>(32, config_.streaming_ph_points / 10));
    ...
    {
        err +=
            std::abs(static_cast<double>(exact[i].second) - static_cast<double>(approx[i].second));
    }
...
{
    std::vector<std::vector<float>> points(num_points, std::vector<float>(kPointDim, 0.0f));
...
std::vector<std::vector<float>> StreamingPHMicrobenchmark::injectAdversarialBursts(
    const std::vector<std::vector<float>> &base_series, std::size_t num_bursts)
{
...
StreamingPHMicrobenchmark::computeExactPh(const std::vector<std::vector<float>> &points)
{
...
IncrementalLaplacianMicrobenchmark::IncrementalLaplacianMicrobenchmark(
    ...
    , rng_(config.random_seed)
{
    incremental_laplacian_ = std::make_unique<optimization::AcceleratedIncrementalLaplacian>(
        optimization::AcceleratedIncrementalLaplacian::LaplacianConfig{});
}

MicrobenchmarkResult IncrementalLaplacianMicrobenchmark::runIncrementalLaplacianBenchmark()
{
    ...
    auto updates = generateIncrementalUpdates(matrix, config_.incremental_updates);
    ...
    return makeResult("incremental_laplacian", std::move(latencies_us), start, end);
}

bool IncrementalLaplacianMicrobenchmark::validateResiduals(const std::vector<double> &residuals)
{
    if (residuals.empty())
    {
        return false;
...
IncrementalLaplacianMicrobenchmark::generateTestMatrix(std::size_t size)
{
    ...
    std::uniform_int_distribution<uint32_t> node(0, static_cast<uint32_t>(size - 1));
    for (auto &row : matrix)
    {
...
std::vector<double> IncrementalLaplacianMicrobenchmark::generateTestWeights(std::size_t size)
{
...
std::vector<std::vector<uint32_t>> IncrementalLaplacianMicrobenchmark::generateIncrementalUpdates(
    const std::vector<std::vector<uint32_t>> &base_matrix, std::size_t num_updates)
{
    ...
    std::uniform_int_distribution<std::size_t> row_dist(0, base_matrix.size() - 1);
