GPUPrimitivesMicrobenchmark::GPUPrimitivesMicrobenchmark(const MicrobenchmarkConfig &config)
    : config_(config)
    , rng_(config.random_seed)
{
    ...
    {
        gpu_cfg.min_batch_size =
            *std::min_element(config_.gpu_batch_sizes.begin(), config_.gpu_batch_sizes.end());
        gpu_cfg.max_batch_size =
            *std::max_element(config_.gpu_batch_sizes.begin(), config_.gpu_batch_sizes.end());
        gpu_cfg.optimal_batch_size = config_.gpu_batch_sizes.front();
    }
    gpu_cfg.num_streams = static_cast<int>(std::max<std::size_t>(1, config_.num_threads));
    gpu_cfg.enable_async_operations = true;
    gpu_cfg.enable_one_kernel_pipeline = true;
    gpu_primitives_ = std::make_unique<optimization::AcceleratedGpuPrimitives>(gpu_cfg);
}

std::vector<MicrobenchmarkResult> GPUPrimitivesMicrobenchmark::runGpuBenchmarks()
{
    std::vector<MicrobenchmarkResult> out;

    const std::size_t n_points = std::max<std::size_t>(64, config_.streaming_ph_points / 8);
    for (const std::size_t batch_size : config_.gpu_batch_sizes)
    {
        ...
        for (std::size_t iter = 0; iter < config_.gpu_iterations; ++iter)
        {
            ...
            for (std::size_t b = 0; b < batch_size; ++b)
            {
                ...
                gpu_primitives_->computeDistanceMatrixBatch(buffers[b], n_points, kPointDim);
            }
        ...
        auto result = makeResult("gpu_distance_batch_n" + std::to_string(n_points) + "_b" +
    ...
    for (std::size_t i = 0; i < config_.gpu_iterations; ++i)
    {
        ...
        const auto t1 = std::chrono::steady_clock::now();
        reduction_latencies.push_back(std::chrono::duration<double, std::micro>(t1 - t0).count());
    }
    const auto reduce_end = std::chrono::steady_clock::now();
    out.push_back(makeResult("gpu_column_reduction", std::move(reduction_latencies), reduce_start,
                             reduce_end));
...
GPUPrimitivesMicrobenchmark::generateTestData(std::size_t batch_size, std::size_t num_points)
{
...
std::vector<float *> GPUPrimitivesMicrobenchmark::allocateGpuBuffers(std::size_t batch_size,
...
void GPUPrimitivesMicrobenchmark::deallocateGpuBuffers(const std::vector<float *> &buffers)
{
...
CompactSummaryMicrobenchmark::CompactSummaryMicrobenchmark(const MicrobenchmarkConfig &config)
    : config_(config)
    , rng_(config.random_seed)
{
...
MicrobenchmarkResult CompactSummaryMicrobenchmark::runCompactSummaryBenchmark()
{
    auto windows = generateTestWindows(config_.compact_summary_windows, config_.window_size);
    std::vector<double> latencies_us;
    ...
    auto result = makeResult("compact_summary", std::move(latencies_us), start, end);
    result.metrics["window_size"] = static_cast<double>(config_.window_size);
    result.metrics["window_count"] = static_cast<double>(config_.compact_summary_windows);
    return result;
}

bool CompactSummaryMicrobenchmark::validatePerformance(const MicrobenchmarkResult &result)
{
...
std::vector<std::vector<std::vector<float>>>
CompactSummaryMicrobenchmark::generateTestWindows(std::size_t count, std::size_t window_size)
{
...
CompactSummaryMicrobenchmark::computeSummaryReference(const std::vector<std::vector<float>> &points)
{
