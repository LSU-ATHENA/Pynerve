...
__global__ void aggregateBucketsKernel(
    const double *__restrict__ predicted_times, const double *__restrict__ observed_times,
    const int *__restrict__ bucket_ids, double *__restrict__ bucket_predicted_sum,
    double *__restrict__ bucket_observed_sum, double *__restrict__ bucket_error_sum,
    int *__restrict__ bucket_counts, int num_samples, int num_buckets)
{
...
__global__ void computeConfidenceKernel(const double *__restrict__ errors,
                                        ...
                                        double *__restrict__ error_bounds, int num_buckets,
                                        int min_samples, double confidence_threshold)
{
    ...
    double scaled_confidence = fmax(0.0, fmin(1.0, raw_confidence * sample_factor));
    confidence[bucket] = scaled_confidence >= confidence_threshold ? scaled_confidence : 0.0;
...
__global__ void modelUpdateKernel(const float *__restrict__ features,
                                  const float *__restrict__ targets, float *__restrict__ weights,
                                  float *__restrict__ bias, int num_samples, int num_features,
