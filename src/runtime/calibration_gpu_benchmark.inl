CalibrationBenchmark benchmarkCalibration(int num_samples, int num_buckets) {
  ...
  for (int i = 0; i < num_samples; ++i) {
    samples.push_back(
        {"hw_" + std::to_string(i % 10), "prob_" + std::to_string(i % 20),
  ...
  auto bucket_id = [num_buckets](const auto &s) {
    ...
    const size_t h = hasher(s.hardware_fingerprint) ^ hasher(s.problem_bucket) ^
                     hasher(s.algorithm);
  ...
  for (const auto &sample : samples) {
    ...
    bucket.error_sum +=
        std::abs(sample.predicted_time_ms - sample.observed_time_ms);
    bucket.count += 1;
  }
  for (auto &bucket : cpu_buckets) {
    ...
    const double sample_factor =
        static_cast<double>(bucket.count) / (bucket.count + 5);
  ...
  bench.cpu_update_ms = std::chrono::duration<double, std::milli>(
                            end_cpu_update - start_cpu_update)
                            .count();
  ...
  bench.gpu_update_ms =
      std::chrono::duration<double, std::milli>(end_update - start_update)
          .count();

  auto start_cpu_query = std::chrono::high_resolution_clock::now();
  const GPUCalibrationModel::Sample query_sample{"hw_0", "prob_0", "alg_0", 0.0,
                                                 0.0,    0.0,      0.0};
  ...
  bench.cpu_query_ms =
      std::chrono::duration<double, std::milli>(end_cpu_query - start_cpu_query)
          .count();
  ...
  bench.gpu_query_ms =
      std::chrono::duration<double, std::milli>(end_query - start_query)
          .count();
  ...
  std::vector<std::vector<float>> features(fit_samples,
                                           std::vector<float>(feature_count));
  ...
  for (int epoch = 0; epoch < 100; ++epoch) {
    ...
    for (int f = 0; f < feature_count; ++f) {
      cpu_weights[f] -=
          0.01f * ((gradients[f] / fit_samples) + 0.001f * cpu_weights[f]);
    }
    cpu_bias -= 0.01f * (bias_grad / fit_samples);
  }
  auto end_cpu_fit = std::chrono::high_resolution_clock::now();
  bench.cpu_fit_ms =
      std::chrono::duration<double, std::milli>(end_cpu_fit - start_cpu_fit)
          .count();
  ...
  bench.gpu_fit_ms =
      std::chrono::duration<double, std::milli>(end_gpu_fit - start_gpu_fit)
          .count();
