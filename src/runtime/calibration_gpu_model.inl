...
public:
  ...
  explicit GPUCalibrationModel(int max_buckets = 1000)
      : max_buckets_(max_buckets), cublas_handle_(nullptr),
        d_bucket_predicted_sum_(nullptr), d_bucket_observed_sum_(nullptr),
        d_bucket_error_sum_(nullptr), d_bucket_counts_(nullptr), d_confidence_(nullptr),
  ...
  void addSamples(const std::vector<Sample> &samples) {
    ...
    for (const auto &s : samples) {
      ...
      int bucket =
          getBucketId(s.hardware_fingerprint, s.problem_bucket, s.algorithm);
    ...
    try {
      ...
      computeConfidenceKernel<<<conf_blocks, BLOCK_SIZE>>>(
          d_bucket_error_sum_, d_bucket_counts_, d_confidence_, d_error_bounds_,
          max_buckets_, 20, 0.8);
      GPU_CHECK(cudaPeekAtLastError());
  ...
  BucketStats queryBucket(const std::string &hardware,
    ...
    GPU_CHECK(cudaMemcpy(&count, &d_bucket_counts_[bucket], sizeof(int),
                         cudaMemcpyDeviceToHost));
    ...
    if (!std::isfinite(stats.predicted_time_avg) ||
        !std::isfinite(stats.observed_time_avg) ||
  ...
  void fitLinearModel(const std::vector<std::vector<float>> &features,
    ...
    try {
      ...
      for (int epoch = 0; epoch < num_epochs; ++epoch) {
        modelUpdateKernel<<<blocks, BLOCK_SIZE>>>(
            d_features, d_targets, d_weights, d_bias, num_samples, num_features,
            learning_rate, 0.001f);
        GPU_CHECK(cudaPeekAtLastError());
      }

      GPU_CHECK(cudaMemcpy(h_weights.data(), d_weights,
...
private:
  int getBucketId(const std::string &hw, const std::string &prob,
