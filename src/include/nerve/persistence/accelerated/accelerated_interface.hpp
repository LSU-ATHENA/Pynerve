...
compute_vr_persistence_accelerated(const core::BufferView<const double> &points, Size point_dim,
                                   const VRConfig &config)
{
...
inline errors::ErrorResult<std::vector<Pair>>
compute_vr_persistence_fast(const core::BufferView<const double> &points, Size point_dim,
                            const VRConfig &config)
{
    return computeVrPersistenceFast(points, point_dim, config);
}

inline VRConfig create_optimal_config(const core::BufferView<const double> &points, Size point_dim,
                                      const VRConfig &base_config)
{
