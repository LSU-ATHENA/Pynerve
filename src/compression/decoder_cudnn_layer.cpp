...
class CuDNNDecoderLayer
{
public:
    CuDNNDecoderLayer(cudnnHandle_t cudnn, cublasHandle_t cublas, int batch_size, int input_dim,
                      int output_dim)
        : cudnn_(cudnn)
        , cublas_(cublas)
        ...
        , output_dim_(output_dim)
    {
        ...
        {
            throw std::invalid_argument("decoder layer requires valid cuDNN and cuBLAS handles");
        }
        ...
        try
        {
            ...
            checkCuda(cudaMalloc(reinterpret_cast<void **>(&d_bias_),
                                 checkedFloatBytes(static_cast<std::size_t>(output_dim_),
                                                   "decoder bias bytes")),
                      "allocate decoder bias");
            checkCuda(cudaMalloc(reinterpret_cast<void **>(&d_output_),
                                 checkedFloatBytes(output_count, "decoder output bytes")),
                      "allocate decoder output");
    ...
    CuDNNDecoderLayer(const CuDNNDecoderLayer &) = delete;
    CuDNNDecoderLayer &operator=(const CuDNNDecoderLayer &) = delete;

    CuDNNDecoderLayer(CuDNNDecoderLayer &&other) noexcept { takeFrom(other); }

    CuDNNDecoderLayer &operator=(CuDNNDecoderLayer &&other) noexcept
    {
        if (this != &other)
        {
            release();
    ...
    ~CuDNNDecoderLayer() { release(); }

    float *forward(const float *d_input, cudnnActivationDescriptor_t activation)
    {
        if (d_input == nullptr)
        {
            throw std::invalid_argument("decoder input device pointer must not be null");
        }
        ...
        const std::size_t output_count =
            checkedElements(batch_size_, output_dim_, "decoder layer output count");
        validateDeviceFloats(d_output_, output_count, "decoder affine output");

        if (activation != nullptr)
        {
            checkCudnn(cudnnActivationForward(cudnn_, activation, &alpha, output_desc_, d_output_,
                                              &beta, output_desc_, d_output_),
                       "run decoder activation");
            validateDeviceFloats(d_output_, output_count, "decoder activation output");
        }
    ...
    void setWeights(const std::vector<float> &weights, const std::vector<float> &bias)
    {
        ...
        requireFiniteValues(weights, "decoder weights");
        requireFiniteValues(bias, "decoder bias");

        checkCuda(cudaMemcpy(d_weights_, weights.data(),
                             checkedFloatBytes(weights.size(), "decoder weight copy bytes"),
                             cudaMemcpyHostToDevice),
                  "copy decoder weights");
        checkCuda(cudaMemcpy(d_bias_, bias.data(),
                             checkedFloatBytes(bias.size(), "decoder bias copy bytes"),
                             cudaMemcpyHostToDevice),
                  "copy decoder bias");
    }

    int getOutputSize() const noexcept { return output_dim_; }
    float *getOutput() const noexcept { return d_output_; }

private:
    void takeFrom(CuDNNDecoderLayer &other) noexcept
    {
        cudnn_ = other.cudnn_;
    ...
    {
        std::mt19937 gen(42);
        float scale = checkedInitialScale(input_dim_, output_dim_, "decoder");
        std::normal_distribution<float> dist(0.0f, scale);

        const std::size_t weight_count =
            checkedElements(input_dim_, output_dim_, "decoder initial weight count");
