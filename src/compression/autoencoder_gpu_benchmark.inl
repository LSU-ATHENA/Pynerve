AutoencoderBenchmark benchmarkAutoencoder(int input_dim, int latent_dim,
                                          int batch_size) {
  ...
  auto cpu_latent =
      cpu_forward(input, batch_size, config.encoder_layers, false);
  auto end_cpu_encode = std::chrono::high_resolution_clock::now();
  bench.cpu_encode_ms = std::chrono::duration<double, std::milli>(
                            end_cpu_encode - start_cpu_encode)
                            .count();
  ...
  bench.gpu_encode_ms =
      std::chrono::duration<double, std::milli>(end_encode - start_encode)
          .count();
  ...
  bench.gpu_decode_ms =
      std::chrono::duration<double, std::milli>(end_decode - start_decode)
          .count();

  auto start_cpu_decode = std::chrono::high_resolution_clock::now();
  auto cpu_output =
      cpu_forward(cpu_latent, batch_size, config.decoder_layers, true);
  auto end_cpu_decode = std::chrono::high_resolution_clock::now();
  bench.cpu_decode_ms = std::chrono::duration<double, std::milli>(
                            end_cpu_decode - start_cpu_decode)
                            .count();
