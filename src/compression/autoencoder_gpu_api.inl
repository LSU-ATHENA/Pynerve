...
public:
  ...
  ~BenchmarkAutoencoder() {
    releaseDeviceState();
  }

  std::vector<float> encode(const std::vector<float> &input_batch) {
    const std::size_t expected_values =
        checkedElementCount(config_.batch_size, config_.encoder_layers.front(), "encoder input");
    ...
    try {
      ...
      for (size_t i = 0; i < d_encoder_weights_.size(); ++i) {
        ...
        checkCublasStatus(cublasSgemm(cublas_handle_, CUBLAS_OP_T, CUBLAS_OP_N, out_dim,
                                      batch_size, in_dim, &alpha, d_encoder_weights_[i], in_dim,
                                      current, in_dim, &beta, d_output, out_dim),
                          "encoder cublasSgemm");
        ...
        if (d_encoder_bias_[i] != nullptr) {
          ...
          addBiasKernel<float><<<blocks, BLOCK_SIZE>>>(d_output, d_encoder_bias_[i], batch_size,
                                                       out_dim);
  ...
  std::vector<float> decode(const std::vector<float> &latent_batch) {
    const std::size_t expected_values =
        checkedElementCount(config_.batch_size, config_.decoder_layers.front(), "decoder input");
    ...
    try {
      ...
      for (size_t i = 0; i < d_decoder_weights_.size(); ++i) {
        ...
        float alpha = 1.0f, beta = 0.0f;
        checkCublasStatus(cublasSgemm(cublas_handle_, CUBLAS_OP_T, CUBLAS_OP_N, out_dim,
                                      batch_size, in_dim, &alpha, d_decoder_weights_[i], in_dim,
                                      current, in_dim, &beta, d_output, out_dim),
                          "decoder cublasSgemm");
        validateDeviceFinite(d_output, output_values,
...
private:
  void validateConfig() const {
    if (config_.batch_size <= 0 || config_.encoder_layers.size() < 2 ||
      ...
      throw std::invalid_argument("autoencoder benchmark requires positive batch and layer sizes");
    }
    ...
    if (config_.encoder_layers.back() != config_.decoder_layers.front()) {
      throw std::invalid_argument("encoder output dimension must match decoder input dimension");
    }
  }
  ...
  void allocateWeights() {
    for (size_t i = 0; i + 1 < config_.encoder_layers.size(); ++i) {
      ...
      GPU_CHECK(cudaMemset(d_b, 0, checkedBytes<float>(static_cast<std::size_t>(cols),
                                                       "encoder bias")));
