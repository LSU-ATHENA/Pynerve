template <typename T> __global__ void __launch_bounds__(256) reluKernel(T *__restrict__ data, int n) {
...
template <typename T>
__global__ void __launch_bounds__(256) addBiasKernel(T *__restrict__ data, const T *__restrict__ bias,
  ...
  data[idx] = (isfinite(static_cast<float>(data[idx])) &&
               isfinite(static_cast<float>(bias[feature])) &&
