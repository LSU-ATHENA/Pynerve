...
std::vector<TunedConfig> DistributedTuningCoordinator::tuneAndBroadcast(
    const WorkloadFingerprint& workload,
    bool usePersistence) {
    ...
    if (topology_.worldRank == 0) {
        ...
        for (size_t i = 0; i < localGpus_.size(); ++i) {
            if (usePersistence) {
                ...
                if (cached) {
                    ...
                    localConfigs_[i] = tuner_.tune(specs, analyzeWorkload(
                        workload.nPoints, workload.pointDim, workload.sparsityEstimate));
        ...
        for (const auto& sig : allSignatures) {
            auto it = std::ranges::find_if(uniqueTypes,
                [&sig](const GpuSignature& existing) {
                    return existing.computeCapability == sig.computeCapability &&
                           existing.smCount == sig.smCount &&
                           existing.clockRate == sig.clockRate;
                });
        ...
        for (size_t i = 0; i < uniqueTypes.size(); ++i) {
            ...
            if (!alreadyTuned) {
                ...
                float smRatio = static_cast<float>(uniqueTypes[i].smCount) /
                                static_cast<float>(baseSmCount);

                TunedConfig adjustedConfig = baseConfig;
                adjustedConfig.blockSize = std::min(512, std::max(128,
                    static_cast<int>(baseConfig.blockSize * std::sqrt(smRatio))));
                adjustedConfig.tileSize = std::min(256, std::max(32,
                    static_cast<int>(baseConfig.tileSize * smRatio)));
                adjustedConfig.clusterSize = std::min(8, std::max(1,
                    static_cast<int>(baseConfig.clusterSize * smRatio)));
    ...
    MPI_Allgather(
        &localConfig, sizeof(ConfigBuffer), MPI_BYTE,
        gathered.data(), sizeof(ConfigBuffer), MPI_BYTE,
        comm_);
...
std::vector<TunedConfig> DistributedTuningCoordinator::tuneHeterogeneous(
    const WorkloadFingerprint& workload,
    bool usePersistence) {
        ...
        if (tuningAssignments[i] == topology_.worldRank) {
            ...
            if (usePersistence) {
                ...
                if (cached) {
                    ...
                    config = tuner_.tune(specs, analyzeWorkload(
                        workload.nPoints, workload.pointDim, workload.sparsityEstimate));
    ...
    for (const auto& sig : localGpus_) {
        auto it = std::ranges::find_if(uniqueTypes,
            [&sig](const GpuSignature& type) {
                return type.computeCapability == sig.computeCapability &&
                       type.smCount == sig.smCount &&
...
std::vector<TunedConfig> DistributedTuningCoordinator::tuneCollective(
    const WorkloadFingerprint& workload,
    ...
    const uint64_t localWork = std::accumulate(
        workloadDistribution.begin(), workloadDistribution.end(), uint64_t{0});

    std::vector<uint64_t> totalWorkPerRank(topology_.worldSize, 0);
    MPI_Allgather(
        const_cast<uint64_t*>(&localWork),
        1,
        MPI_UINT64_T,
        totalWorkPerRank.data(),
        1,
        MPI_UINT64_T,
        comm_);

    const uint64_t globalWork = std::accumulate(
        totalWorkPerRank.begin(), totalWorkPerRank.end(), uint64_t{0});
    const float averageWork = topology_.worldSize > 0
        ? static_cast<float>(globalWork) / static_cast<float>(topology_.worldSize)
        : 0.0f;
    const float loadScale = averageWork > 0.0f
        ? std::clamp(static_cast<float>(localWork) / averageWork, 0.5f, 2.0f)
        : 1.0f;
    ...
    if (baseConfigs.empty()) {
        ...
        for (size_t i = 0; i < localGpus_.size(); ++i) {
            ...
            tuned.blockSize = std::clamp(
                static_cast<int>(std::lround(base.blockSize * std::sqrt(loadScale))),
                64, 512);
            tuned.tileSize = std::clamp(
                static_cast<int>(std::lround(base.tileSize * loadScale)),
                16, 256);
            tuned.clusterSize = std::clamp(
                static_cast<int>(std::lround(base.clusterSize * loadScale)),
                1, 8);
            tuned.modelConfidence = std::clamp(
                base.modelConfidence * (0.8f + 0.2f / std::max(loadScale, 0.5f)),
                0.0f, 1.0f);
    ...
    MPI_Allgather(
        &localConfig, sizeof(ConfigBuffer), MPI_BYTE,
        rankConfigs.data(), sizeof(ConfigBuffer), MPI_BYTE,
        comm_);
