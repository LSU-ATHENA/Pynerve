...
MultiGpuWorkDistributor::distributeByTopology(
    uint32_t totalWork,
    ...
    for (float p : performance) totalPerformance += p;
...
MultiGpuWorkDistributor::WorkDistribution
MultiGpuWorkDistributor::distributeBandwidthAware(
    uint32_t totalWork,
    const NvLinkTopology& topology,
    ...
    for (float bw : bandwidths) totalBandwidth += bw;
...
void MultiGpuSynchronizer::allGather(
    void* localData,
    size_t localSize,
    std::vector<void*>& allData,
    const NvLinkTopology& topology) {
    if (localSize == 0) {
        return;
    }
    ...
    for (size_t src = 0; src < deviceIds_.size(); ++src) {
        for (size_t dst = 0; dst < deviceIds_.size(); ++dst) {
            ...
            if (topology.hasP2P(srcDevice, dstDevice)) {
                ...
                throwOnCudaFailure(cudaMemcpyPeer(allData[dst], dstDevice, localData, srcDevice,
                                                  localSize),
...
void MultiGpuSynchronizer::broadcast(
    void* data,
    size_t size,
    int rootDevice,
    const NvLinkTopology& topology) {
    ...
    for (size_t i = 0; i < deviceIds_.size(); ++i) {
        if (static_cast<int>(i) == rootDevice) continue;
