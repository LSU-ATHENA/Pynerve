std::vector<NodeGpuConfig> DistributedTuningCoordinator::gatherAllConfigs() {
    ...
    *reinterpret_cast<int*>(&sendBuffer[offset]) = numGpus;
    ...
    MPI_Gather(&sendCount, 1, MPI_INT,
               recvCounts.data(), 1, MPI_INT, 0, comm_);
    ...
    MPI_Gatherv(sendBuffer.data(), sendCount, MPI_BYTE,
                recvBuffer.data(), recvCounts.data(), displs.data(), MPI_BYTE,
                0, comm_);
...
bool DistributedTuningCoordinator::isHomogeneous() const {
    ...
    for (const auto& sig : allSignatures) {
        if (sig.computeCapability != first.computeCapability ||
            sig.smCount != first.smCount ||
...
void DistributedTuningCoordinator::gatherSignatures(std::vector<GpuSignature>& allSignatures) {
    ...
    if (topology_.worldRank == 0) {
        int total = 0;
        for (int c : allCounts) total += c * 5;
        recvBuffer.resize(total);
    }

    MPI_Gatherv(localBuffer.data(), localBufSize, MPI_INT,
                recvBuffer.data(),
...
void DistributedTuningCoordinator::scatterConfigs(const std::vector<TunedConfig>& allConfigs) {
    ...
    MPI_Scatter(buffers.data(), sizeof(ConfigBuffer), MPI_BYTE,
                buffers.data(), sizeof(ConfigBuffer), MPI_BYTE,
                0, comm_);
...
std::vector<GpuSignature> DistributedTuningCoordinator::findUniqueGpuTypes() {
    ...
    for (const auto& sig : allSignatures) {
        auto it = std::ranges::find_if(unique,
...
std::vector<int> DistributedTuningCoordinator::assignTuningWork(
    const std::vector<GpuSignature>& uniqueTypes) {
