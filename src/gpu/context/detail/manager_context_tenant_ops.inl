...
WorkloadHandle MultiTenantManager::submitWorkload(const WorkloadDescription &desc,
                                                  GreenContextId preferredContext)
{
    ...
    if (handle.assignedContext == 0)
    {
        const size_t headroom = desc.memoryRequirement > std::numeric_limits<size_t>::max() / 2
                                    ? std::numeric_limits<size_t>::max()
                                    : desc.memoryRequirement * 2;
        auto ctx = contextManager_.createContext(desc.computeFraction, headroom, desc.priority);
...
GreenContextId MultiTenantManager::findBestContext(const WorkloadDescription &desc)
{
    ...
    for (auto ctxId : activeContexts)
    {
        ...
        float memoryAvailable =
            1.0f - (static_cast<float>(metrics.memoryUsed) / metrics.memoryLimit);
...
std::vector<WorkloadHandle> MultiTenantManager::getActiveWorkloads() const
{
    ...
    {
        if (handle.status == WorkloadStatus::kPending || handle.status == WorkloadStatus::kRunning)
        {
...
void MultiTenantManager::setRebalancingPolicy(bool autoRebalance, int rebalanceIntervalMs)
{
    autoRebalance_ = autoRebalance;
    rebalanceIntervalMs_ = std::max(1, rebalanceIntervalMs);
}

QoSManager::QoSManager(GreenContextManager &manager)
    : manager_(manager)
{}
...
std::string QoSManager::getQoSReport(GreenContextId ctxId) const
{
    ...
    auto metrics = manager_.getContextMetrics(ctxId);
    out << "context=" << ctxId << " active=" << metrics.active
        << " compute=" << metrics.computeUtilization << " memory=" << metrics.memoryUsed << "/"
        << metrics.memoryLimit << " qos_ok=" << checkQoS(ctxId);
    return out.str();
}
...
GreenContext createGreenContext(float computeFraction, size_t memoryLimit, int priority)
{
