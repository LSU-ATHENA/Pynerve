...
void memcpy(void *dst, const void *src, std::size_t bytes) { std::memcpy(dst, src, bytes); }
void memset(void *dst, int value, std::size_t bytes) { std::memset(dst, value, bytes); }
void add(double *a, const double *b, std::size_t n) { for (std::size_t i = 0; i < n; ++i) a[i] += b[i]; }
void sub(double *a, const double *b, std::size_t n) { for (std::size_t i = 0; i < n; ++i) a[i] -= b[i]; }
void mul(double *a, const double *b, std::size_t n) { for (std::size_t i = 0; i < n; ++i) a[i] *= b[i]; }
void scale(double *a, double alpha, std::size_t n) { for (std::size_t i = 0; i < n; ++i) a[i] *= alpha; }
void axpy(double alpha, const double *x, double *y, std::size_t n) { for (std::size_t i = 0; i < n; ++i) y[i] += alpha * x[i]; }
void fmad(const double *a, const double *b, double *c, std::size_t n) { for (std::size_t i = 0; i < n; ++i) c[i] += a[i] * b[i]; }
double reduce_sum(const double *data, std::size_t n) { double s = 0; for (std::size_t i = 0; i < n; ++i) s += data[i]; return s; }
double reduce_max(const double *data, std::size_t n) { if (n == 0) return 0; double m = data[0]; for (std::size_t i = 1; i < n; ++i) if (data[i] > m) m = data[i]; return m; }
double reduce_min(const double *data, std::size_t n) { if (n == 0) return 0; double m = data[0]; for (std::size_t i = 1; i < n; ++i) if (data[i] < m) m = data[i]; return m; }
double dot(const double *a, const double *b, std::size_t n) { double s = 0; for (std::size_t i = 0; i < n; ++i) s += a[i] * b[i]; return s; }
double norm2(const double *vec, std::size_t n) { double s = 0; for (std::size_t i = 0; i < n; ++i) s += vec[i] * vec[i]; return std::sqrt(s); }
double sqdiff_sum(const double *a, const double *b, std::size_t n) { double s = 0; for (std::size_t i = 0; i < n; ++i) { double d = a[i] - b[i]; s += d * d; } return s; }
void abs(double *data, std::size_t n) { for (std::size_t i = 0; i < n; ++i) data[i] = std::abs(data[i]); }
void neg(double *data, std::size_t n) { for (std::size_t i = 0; i < n; ++i) data[i] = -data[i]; }
void sqrt(double *data, std::size_t n) { for (std::size_t i = 0; i < n; ++i) data[i] = std::sqrt(data[i]); }
void exp(double *data, std::size_t n) { for (std::size_t i = 0; i < n; ++i) data[i] = std::exp(data[i]); }
void log(double *data, std::size_t n) { for (std::size_t i = 0; i < n; ++i) data[i] = std::log(data[i]); }
void relu(double *data, std::size_t n) { for (std::size_t i = 0; i < n; ++i) if (data[i] < 0) data[i] = 0; }
void sigmoid(double *data, std::size_t n) { for (std::size_t i = 0; i < n; ++i) data[i] = 1.0 / (1.0 + std::exp(-data[i])); }
void tanh(double *data, std::size_t n) { for (std::size_t i = 0; i < n; ++i) data[i] = std::tanh(data[i]); }
void min(double *a, const double *b, std::size_t n) { for (std::size_t i = 0; i < n; ++i) a[i] = a[i] < b[i] ? a[i] : b[i]; }
void max(double *a, const double *b, std::size_t n) { for (std::size_t i = 0; i < n; ++i) a[i] = a[i] > b[i] ? a[i] : b[i]; }
void clamp(double *data, double lo, double hi, std::size_t n) { for (std::size_t i = 0; i < n; ++i) { if (data[i] < lo) data[i] = lo; if (data[i] > hi) data[i] = hi; } }
void fmad_f32(const float *a, const float *b, float *c, std::size_t n) { for (std::size_t i = 0; i < n; ++i) c[i] += a[i] * b[i]; }
void add_f32(float *a, const float *b, std::size_t n) { for (std::size_t i = 0; i < n; ++i) a[i] += b[i]; }
void sub_f32(float *a, const float *b, std::size_t n) { for (std::size_t i = 0; i < n; ++i) a[i] -= b[i]; }
void mul_f32(float *a, const float *b, std::size_t n) { for (std::size_t i = 0; i < n; ++i) a[i] *= b[i]; }
void scale_f32(float *a, float alpha, std::size_t n) { for (std::size_t i = 0; i < n; ++i) a[i] *= alpha; }
void axpy_f32(float alpha, const float *x, float *y, std::size_t n) { for (std::size_t i = 0; i < n; ++i) y[i] += alpha * x[i]; }
float reduce_sum_f32(const float *data, std::size_t n) { float s = 0; for (std::size_t i = 0; i < n; ++i) s += data[i]; return s; }
float reduce_max_f32(const float *data, std::size_t n) { if (n == 0) return 0; float m = data[0]; for (std::size_t i = 1; i < n; ++i) if (data[i] > m) m = data[i]; return m; }
float reduce_min_f32(const float *data, std::size_t n) { if (n == 0) return 0; float m = data[0]; for (std::size_t i = 1; i < n; ++i) if (data[i] < m) m = data[i]; return m; }
float dot_f32(const float *a, const float *b, std::size_t n) { float s = 0; for (std::size_t i = 0; i < n; ++i) s += a[i] * b[i]; return s; }
float norm2_f32(const float *vec, std::size_t n) { float s = 0; for (std::size_t i = 0; i < n; ++i) s += vec[i] * vec[i]; return std::sqrt(s); }
void neg_f32(float *data, std::size_t n) { for (std::size_t i = 0; i < n; ++i) data[i] = -data[i]; }
void sqrt_f32(float *data, std::size_t n) { for (std::size_t i = 0; i < n; ++i) data[i] = std::sqrt(data[i]); }
void exp_f32(float *data, std::size_t n) { for (std::size_t i = 0; i < n; ++i) data[i] = std::exp(data[i]); }
void log_f32(float *data, std::size_t n) { for (std::size_t i = 0; i < n; ++i) data[i] = std::log(data[i]); }
void sigmoid_f32(float *data, std::size_t n) { for (std::size_t i = 0; i < n; ++i) data[i] = 1.0f / (1.0f + std::exp(-data[i])); }
void tanh_f32(float *data, std::size_t n) { for (std::size_t i = 0; i < n; ++i) data[i] = std::tanh(data[i]); }
float sqdiff_sum_f32(const float *a, const float *b, std::size_t n) { float s = 0; for (std::size_t i = 0; i < n; ++i) { float d = a[i] - b[i]; s += d * d; } return s; }
float euclidean_f32(const float *a, const float *b, std::size_t n) { return std::sqrt(sqdiff_sum_f32(a, b, n)); }
float cosine_f32(const float *a, const float *b, std::size_t n) {
...
void abs_f32(float *data, std::size_t n) { for (std::size_t i = 0; i < n; ++i) data[i] = std::abs(data[i]); }
void relu_f32(float *data, std::size_t n) { for (std::size_t i = 0; i < n; ++i) if (data[i] < 0) data[i] = 0; }
void min_f32(float *a, const float *b, std::size_t n) { for (std::size_t i = 0; i < n; ++i) a[i] = a[i] < b[i] ? a[i] : b[i]; }
void max_f32(float *a, const float *b, std::size_t n) { for (std::size_t i = 0; i < n; ++i) a[i] = a[i] > b[i] ? a[i] : b[i]; }
void clamp_f32(float *data, float lo, float hi, std::size_t n) { for (std::size_t i = 0; i < n; ++i) { if (data[i] < lo) data[i] = lo; if (data[i] > hi) data[i] = hi; } }
void add_f16(half *a, const half *b, std::size_t n) { for (std::size_t i = 0; i < n; ++i) a[i] = float_to_half(half_to_float(a[i]) + half_to_float(b[i])); }
void sub_f16(half *a, const half *b, std::size_t n) { for (std::size_t i = 0; i < n; ++i) a[i] = float_to_half(half_to_float(a[i]) - half_to_float(b[i])); }
void mul_f16(half *a, const half *b, std::size_t n) { for (std::size_t i = 0; i < n; ++i) a[i] = float_to_half(half_to_float(a[i]) * half_to_float(b[i])); }
void scale_f16(half *a, half alpha, std::size_t n) { float fa = half_to_float(alpha); for (std::size_t i = 0; i < n; ++i) a[i] = float_to_half(half_to_float(a[i]) * fa); }
void axpy_f16(half alpha, const half *x, half *y, std::size_t n) { float fa = half_to_float(alpha); for (std::size_t i = 0; i < n; ++i) y[i] = float_to_half(half_to_float(y[i]) + fa * half_to_float(x[i])); }
void fmad_f16(const half *a, const half *b, half *c, std::size_t n) { for (std::size_t i = 0; i < n; ++i) c[i] = float_to_half(half_to_float(c[i]) + half_to_float(a[i]) * half_to_float(b[i])); }
float reduce_sum_f16(const half *data, std::size_t n) { float s = 0; for (std::size_t i = 0; i < n; ++i) s += half_to_float(data[i]); return s; }
float reduce_max_f16(const half *data, std::size_t n) { if (n == 0) return 0; float m = half_to_float(data[0]); for (std::size_t i = 1; i < n; ++i) { float v = half_to_float(data[i]); if (v > m) m = v; } return m; }
float reduce_min_f16(const half *data, std::size_t n) { if (n == 0) return 0; float m = half_to_float(data[0]); for (std::size_t i = 1; i < n; ++i) { float v = half_to_float(data[i]); if (v < m) m = v; } return m; }
float dot_f16(const half *a, const half *b, std::size_t n) { float s = 0; for (std::size_t i = 0; i < n; ++i) s += half_to_float(a[i]) * half_to_float(b[i]); return s; }
float norm2_f16(const half *vec, std::size_t n) { float s = 0; for (std::size_t i = 0; i < n; ++i) { float v = half_to_float(vec[i]); s += v * v; } return std::sqrt(s); }
void neg_f16(half *data, std::size_t n) { for (std::size_t i = 0; i < n; ++i) data[i] = float_to_half(-half_to_float(data[i])); }
void sqrt_f16(half *data, std::size_t n) { for (std::size_t i = 0; i < n; ++i) data[i] = float_to_half(std::sqrt(half_to_float(data[i]))); }
void exp_f16(half *data, std::size_t n) { for (std::size_t i = 0; i < n; ++i) data[i] = float_to_half(std::exp(half_to_float(data[i]))); }
void log_f16(half *data, std::size_t n) { for (std::size_t i = 0; i < n; ++i) data[i] = float_to_half(std::log(half_to_float(data[i]))); }
void relu_f16(half *data, std::size_t n) { for (std::size_t i = 0; i < n; ++i) { float v = half_to_float(data[i]); data[i] = float_to_half(v < 0 ? 0 : v); } }
void sigmoid_f16(half *data, std::size_t n) { for (std::size_t i = 0; i < n; ++i) { float v = half_to_float(data[i]); data[i] = float_to_half(1.0f / (1.0f + std::exp(-v))); } }
void tanh_f16(half *data, std::size_t n) { for (std::size_t i = 0; i < n; ++i) data[i] = float_to_half(std::tanh(half_to_float(data[i]))); }
void abs_f16(half *data, std::size_t n) { for (std::size_t i = 0; i < n; ++i) data[i] = float_to_half(std::abs(half_to_float(data[i]))); }
void min_f16(half *a, const half *b, std::size_t n) { for (std::size_t i = 0; i < n; ++i) { float fa = half_to_float(a[i]); float fb = half_to_float(b[i]); a[i] = float_to_half(fa < fb ? fa : fb); } }
void max_f16(half *a, const half *b, std::size_t n) { for (std::size_t i = 0; i < n; ++i) { float fa = half_to_float(a[i]); float fb = half_to_float(b[i]); a[i] = float_to_half(fa > fb ? fa : fb); } }
void clamp_f16(half *data, half lo, half hi, std::size_t n) { float flo = half_to_float(lo); float fhi = half_to_float(hi); for (std::size_t i = 0; i < n; ++i) { float v = half_to_float(data[i]); if (v < flo) v = flo; if (v > fhi) v = fhi; data[i] = float_to_half(v); } }
float sqdiff_sum_f16(const half *a, const half *b, std::size_t n) { float s = 0; for (std::size_t i = 0; i < n; ++i) { float d = half_to_float(a[i]) - half_to_float(b[i]); s += d * d; } return s; }
void gemv_f32(float alpha, const float *A, const float *x, float beta, float *y, std::size_t m, std::size_t n) {
    if (beta != 1.0f) for (std::size_t i = 0; i < m; ++i) y[i] *= beta;
    for (std::size_t i = 0; i < m; ++i) { float s = 0; for (std::size_t j = 0; j < n; ++j) s += A[i * n + j] * x[j]; y[i] += alpha * s; }
}
void ger_f32(float alpha, const float *x, const float *y, float *A, std::size_t m, std::size_t n) {
    for (std::size_t i = 0; i < m; ++i) for (std::size_t j = 0; j < n; ++j) A[i * n + j] += alpha * x[i] * y[j];
}
void gemv_f16(half alpha, const half *A, const half *x, half beta, half *y, std::size_t m, std::size_t n) {
    float fa = half_to_float(alpha);
    float fb = half_to_float(beta);
    if (fb != 1.0f) for (std::size_t i = 0; i < m; ++i) y[i] = float_to_half(half_to_float(y[i]) * fb);
    for (std::size_t i = 0; i < m; ++i) {
        float sum = 0.0f;
        for (std::size_t j = 0; j < n; ++j) sum += half_to_float(A[i * n + j]) * half_to_float(x[j]);
        y[i] = float_to_half(half_to_float(y[i]) + fa * sum);
    }
}
void ger_f16(half alpha, const half *x, const half *y, half *A, std::size_t m, std::size_t n) {
    ...
    for (std::size_t i = 0; i < m; ++i) {
        float xi = half_to_float(x[i]);
        for (std::size_t j = 0; j < n; ++j) A[i * n + j] = float_to_half(half_to_float(A[i * n + j]) + fa * xi * half_to_float(y[j]));
    }
}
void quantize_f16(const half *input, std::size_t n, int bits, std::uint8_t *output) {
    if (n == 0) return;
...
void dequantize_f16(const std::uint8_t *input, std::size_t n, int bits, half *output) {
    if (n == 0) return;
    float inv = 1.0f / static_cast<float>((1 << bits) - 1);
    for (std::size_t i = 0; i < n; ++i) output[i] = float_to_half(static_cast<float>(input[i]) * inv);
}
void gemv(double alpha, const double *A, const double *x, double beta, double *y, std::size_t m, std::size_t n) {
    if (beta != 1.0) for (std::size_t i = 0; i < m; ++i) y[i] *= beta;
    for (std::size_t i = 0; i < m; ++i) { double s = 0; for (std::size_t j = 0; j < n; ++j) s += A[i * n + j] * x[j]; y[i] += alpha * s; }
}
