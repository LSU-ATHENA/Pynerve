...
template<Numeric T>
void KNNComputer<T>::find_k_smallest(std::span<T> distances,
...
template<Numeric T>
typename KNNComputer<T>::Result KNNComputer<T>::compute_brute_force(
...
template<Numeric T>
typename KNNComputer<T>::Result KNNComputer<T>::compute_query(
    std::span<const T> reference, size_t n_ref,
...
SparseDistanceMatrixComputer<T>::compute(std::span<const T> points,
                                        size_t n_points,
                                        size_t dim) const {
    validate_flat_matrix(points, n_points, dim, "points");
    switch (config_.mode) {
...
template<Numeric T>
typename SparseDistanceMatrixComputer<T>::SparseMatrix
SparseDistanceMatrixComputer<T>::compute_epsilon(std::span<const T> points,
                                               size_t n_points,
                                               size_t dim) const {
    validate_flat_matrix(points, n_points, dim, "points");
...
template<Numeric T>
typename SparseDistanceMatrixComputer<T>::SparseMatrix
SparseDistanceMatrixComputer<T>::compute_knn(std::span<const T> points,
                                             size_t n_points,
                                             size_t dim) const {
    validate_flat_matrix(points, n_points, dim, "points");
...
template<Numeric T>
typename SparseDistanceMatrixComputer<T>::SparseMatrix
SparseDistanceMatrixComputer<T>::from_dense(std::span<const T> dense,
                                            size_t n_rows,
                                            size_t n_cols,
                                            T threshold) {
...
std::vector<T> SparseDistanceMatrixComputer<T>::to_dense(const SparseMatrix& sparse) {
    ...
    for (size_t i = 0; i < sparse.n_rows; ++i) {
        if (sparse.row_ptr[i] > sparse.row_ptr[i + 1] || sparse.row_ptr[i + 1] > sparse.values.size()) {
