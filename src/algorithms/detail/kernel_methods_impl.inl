...
template <nerve::algorithms::Numeric T>
std::vector<std::span<const T>> make_diagram_spans(const T *const *diagrams, const size_t *sizes,
                                                   size_t n_diagrams)
{
    if (n_diagrams == 0)
    {
...
public:
    ...
    virtual T compute(std::span<const T> d1, size_t n1, std::span<const T> d2, size_t n2) const = 0;
...
public:
    ...
    explicit GaussianKernel(const Config &config)
        : config_(config)
    {
    ...
    T compute(std::span<const T> d1, size_t n1, std::span<const T> d2, size_t n2) const override
    {
...
private:
    ...
    T compute_distance(std::span<const T> d1, size_t n1, std::span<const T> d2, size_t n2) const
    {
    ...
    T euclidean_distance(std::span<const T> d1, size_t n1, std::span<const T> d2, size_t n2) const
    {
        T total_dist = T(0);
        size_t count = 0;
    ...
    T wasserstein_distance(std::span<const T> d1, size_t n1, std::span<const T> d2, size_t n2,
                           T p) const
    {
        if (n1 == 0 && n2 == 0)
            return T(0);
    ...
    T bottleneck_distance(std::span<const T> d1, size_t n1, std::span<const T> d2, size_t n2) const
    {
        if (n1 == 0 && n2 == 0)
            return T(0);
        if (n1 == 0 || n2 == 0)
...
public:
    ...
    explicit PersistenceScaleSpaceKernel(const Config &config)
        : config_(config)
    {
        if (config_.sigma <= T(0) || !std::isfinite(config_.sigma))
        {
...
public:
    ...
    explicit SlicedWassersteinKernel(const Config &config)
        : config_(config)
    {
        if (config_.sigma <= T(0) || !std::isfinite(config_.sigma))
        {
