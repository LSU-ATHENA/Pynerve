...
{
    return persistence_image_impl(diagram, resolution_birth, resolution_death, sigma, birth_min,
                                  birth_max, death_min, death_max, weight_fn);
}

at::Tensor ml_persistence_landscape(const at::Tensor &diagram, int64_t k, int64_t num_samples,
...
at::Tensor ml_heat_kernel_signature(const at::Tensor &diagram, int64_t num_samples, double sigma,
                                    const at::Tensor &t_values)
{
    return heat_kernel_signature_impl(diagram, num_samples, sigma, t_values);
}

at::Tensor ml_birth_death_curve(const at::Tensor &diagram, int64_t num_bins,
