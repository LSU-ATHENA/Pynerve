...
{
    return require_point_matrix(tensor, name).to(at::kCPU).to(at::kDouble).contiguous();
}
...
pull_back_ball_cover(const at::Tensor &point_cloud, const at::Tensor &filter_values,
...
std::vector<std::vector<int64_t>>
normalized_clusters(const std::vector<std::vector<int64_t>> &clusters)
{
                                                          ...
                                                          int64_t resolution, double overlap)
{
    ...
    auto emit_cell = [&](auto &&self, int64_t axis) -> void {
        if (axis == dim)
        {
            ...
            {
                min_bound[d] =
                    min_vals[d] + step[d] * static_cast<double>(cell[d]) - overlap_size[d];
                max_bound[d] =
                    min_vals[d] + step[d] * static_cast<double>(cell[d] + 1) + overlap_size[d];
            }
                                                      ...
                                                      int64_t n_balls, double overlap)
{
    ...
    const auto centers = values.index_select(0, make_index_tensor(center_indices, values.device()));
...
std::vector<std::vector<int64_t>> cluster_single_linkage(const at::Tensor &points, double threshold)
{
...
std::vector<std::vector<int64_t>>
Mapper::pull_back_and_cluster(const at::Tensor &point_cloud, const at::Tensor &filter_values,
    ...
    for (size_t i = 0; i < clean_clusters.size(); ++i)
    {
        for (size_t j = i + 1; j < clean_clusters.size(); ++j)
        {
            ...
            edge.weight =
                ...
                static_cast<double>(std::min(clean_clusters[i].size(), clean_clusters[j].size()));
...
MapperGraph quick_mapper(const at::Tensor &point_cloud, const std::string &filter,
                         int64_t resolution, double overlap)
{
    MapperConfig config;
