at::Tensor PersistenceDiagram::to_persistence_image(
    int64_t resolution,
    double sigma,
    double birth_min,
    double birth_max,
    double death_min,
    double death_max) const {
    ...
    if (death_max < death_min) {
        auto finite_deaths = deaths.masked_select(deaths != std::numeric_limits<double>::infinity());
    ...
    for (int64_t i = 0; i < finite.num_pairs(); ++i) {
        ...
        at::Tensor gaussian = at::exp(
            -(birth_diff.unsqueeze(1).pow(2) + death_diff.unsqueeze(0).pow(2)) /
            (2 * sigma * sigma));
        image += gaussian * (d - b);
    }
...
at::Tensor PersistenceDiagram::to_persistence_landscape(
    int64_t k,
    int64_t num_samples,
    double min_val,
    ...
    at::Tensor triangles = at::clamp_min(
        heights.unsqueeze(1) - at::abs(x.unsqueeze(0) - mids.unsqueeze(1)),
        0.0);
...
at::Tensor PersistenceDiagram::to_betti_curve(
    int64_t num_samples,
    double min_val,
    double max_val) const {
    validate_positive_count(num_samples, "num_samples");
    ...
    if (max_val < min_val) {
        min_val = at::min(births).item<double>();
        auto finite_deaths = deaths.masked_select(deaths != std::numeric_limits<double>::infinity());
        max_val = finite_deaths.numel() > 0 ? at::max(finite_deaths).item<double>() : at::max(births).item<double>();
    }
...
double PersistenceDiagram::wasserstein_distance(
    const PersistenceDiagram& other,
    double p) const {
    TORCH_CHECK(p >= 1.0 && std::isfinite(p),
                "p must be finite and >= 1 for Wasserstein distance");
...
double PersistenceDiagram::persistence_kernel(
    const PersistenceDiagram& other,
    double sigma) const {
    validate_positive_finite_scalar(sigma, "sigma");
...
PersistenceDiagram PersistenceDiagram::threshold(double min_persistence) const {
    ...
    return PersistenceDiagram(
        diagram_.index_select(0, indices),
        dimensions_.index_select(0, indices),
        birth_idx_.index_select(0, indices),
        death_idx_.index_select(0, indices));
}
...
PersistenceDiagram PersistenceDiagram::batch(
...
PersistenceDiagram PersistenceDiagram::get_batch_item(int64_t idx) const {
    ...
    return PersistenceDiagram(
        diagram_.narrow(0, offset, count).clone(),
        dimensions_.narrow(0, offset, count).clone(),
        birth_idx_.narrow(0, offset, count).clone(),
        death_idx_.narrow(0, offset, count).clone());
}
