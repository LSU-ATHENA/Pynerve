...
__global__ void convertFP32toFP64Kernel(const float *__restrict__ input,
  ...
  if (idx < n) {
    ...
    output[idx] = isfinite(value) ? static_cast<double>(value)
                                  : eigensolverInfinityDouble();
  }
}
...
__global__ void convertFP64toFP32Kernel(const double *__restrict__ input,
  ...
  if (idx < n) {
    ...
    output[idx] = isfinite(value) && fabs(value) <= max_float
...
size_t checkedByteCount(size_t count, size_t element_size,
                        const char *context) {
...
int checkedGridBlocks(size_t elements, int block_size, const char *context) {
  const size_t blocks =
      (elements + static_cast<size_t>(block_size) - 1) /
...
void checkSolverInfo(int *d_info, const char *context) {
  ...
  if (info != 0) {
    throw std::runtime_error(std::string(context) +
                             " failed with solver info " + std::to_string(info));
  }
}
...
public:
  GPUEigensolver(int matrix_size, bool use_mixed_precision = true)
      : matrix_size_(matrix_size), use_mixed_precision_(use_mixed_precision) {
    matrix_entries_ = checkedPositiveSquareElements(
        matrix_size_, "GPU eigensolver matrix size must be positive");
    matrix_float_bytes_ = checkedByteCount(
        matrix_entries_, sizeof(float),
        "GPU eigensolver matrix allocation exceeds host limits");
    eigenvalue_float_bytes_ = checkedByteCount(
        static_cast<size_t>(matrix_size_), sizeof(float),
    ...
    if (use_mixed_precision_) {
      ...
      matrix_double_bytes_ = checkedByteCount(
          matrix_entries_, sizeof(double),
          "GPU eigensolver FP64 matrix allocation exceeds host limits");
      eigenvalue_double_bytes_ = checkedByteCount(
          static_cast<size_t>(matrix_size_), sizeof(double),
  ...
  void solve(const std::vector<float> &matrix, std::vector<float> &eigenvalues,
    ...
    GPU_CHECK(cudaMemcpy(d_matrix_, matrix.data(), matrix_float_bytes_,
    ...
    GPU_CHECK(cudaMemcpy(eigenvectors.data(), d_eigenvectors_,
                         eigenvector_float_bytes_, cudaMemcpyDeviceToHost));
   ...
   */
  void solveKSmallest(const std::vector<float> &matrix, int k,
    ...
    std::copy(all_eigenvalues.begin(), all_eigenvalues.begin() + k,
              eigenvalues.begin());
...
private:
  ...
  void solveMixedPrecision() {
    ...
    // Solve in FP64
    int lwork = 0;
    checkCuSolver(cusolverDnDsyevd_bufferSize(cusolver_handle_, CUSOLVER_EIG_MODE_VECTOR,
                                              CUBLAS_FILL_MODE_LOWER, matrix_size_,
                                              d_matrix_fp64_, matrix_size_,
                                              d_eigenvalues_fp64_, &lwork),
    ...
    const size_t fp64_workspace_bytes = checkedByteCount(
