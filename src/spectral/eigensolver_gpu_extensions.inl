...
{
public:
    explicit MultiGPUEigensolver(int num_gpus)
        : num_gpus_(num_gpus)
    {
        if (num_gpus_ > 0)
        {
    ...
    void distributeAndSolve(const std::vector<float> &matrix, std::vector<float> &eigenvalues,
                            std::vector<float> &eigenvectors, int matrix_size)
    {
        ...
        if (best_gpu >= 0)
        {
            eigenvalues = std::move(per_gpu[static_cast<size_t>(best_gpu)].eigenvalues);
            eigenvectors = std::move(per_gpu[static_cast<size_t>(best_gpu)].eigenvectors);
            return;
        }
...
private:
    static double computeResidualScore(const std::vector<float> &matrix,
                                       ...
                                       const std::vector<float> &eigenvectors, int matrix_size)
    {
        size_t matrix_entries = 0;
        try
        {
        ...
        if (matrix.size() != matrix_entries ||
            ...
            eigenvectors.size() < required_vector_entries || !valuesAreFinite(matrix) ||
            !valuesAreFinite(eigenvalues) || !valuesAreFinite(eigenvectors))
        {
            return std::numeric_limits<double>::infinity();
        }
        ...
        for (int eig = 0; eig < pairs_to_check; ++eig)
        {
            ...
            for (int row = 0; row < matrix_size; ++row)
            {
                ...
                for (int col = 0; col < matrix_size; ++col)
                {
                    ...
                    if (!std::isfinite(lhs) || !std::isfinite(rhs) || !std::isfinite(product) ||
                        !std::isfinite(next))
                    {
                        return std::numeric_limits<double>::infinity();
                    }
                ...
                const double scaled = static_cast<double>(lambda) * static_cast<double>(vec[row]);
...
public:
                 ...
                 std::vector<int> &labels)
    {
        ...
        if (eigenvectors.size() < required_eigenvector_entries || !valuesAreFinite(eigenvectors))
        {
            labels.clear();
            return;
        }
        ...
        for (int i = 0; i < matrix_size; ++i)
        {
            ...
            {
                const size_t offset = static_cast<size_t>(c) * static_cast<size_t>(matrix_size) +
...
private:
    ...
    bool computeLaplacian(const std::vector<float> &affinity, std::vector<float> &laplacian, int n)
    {
        ...
        for (int i = 0; i < n; ++i)
        {
            ...
            {
                const size_t offset =
                    static_cast<size_t>(i) * static_cast<size_t>(n) + static_cast<size_t>(j);
                const double next =
        ...
        for (int i = 0; i < n; ++i)
        {
            for (int j = 0; j < n; ++j)
            {
                const size_t offset =
                    static_cast<size_t>(i) * static_cast<size_t>(n) + static_cast<size_t>(j);
                const double value =
                    i == j ? degree[static_cast<size_t>(i)] - static_cast<double>(affinity[offset])
