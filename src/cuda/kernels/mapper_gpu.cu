    ...
    for (int j = 0; j < n_points; ++j)
    {
        ...
        for (int d = 0; d < dim; ++d)
        {
            ...
            if constexpr (std::is_same_v<T, float>)
                dist_sq = ptx::fma_f32(diff, diff, dist_sq);
            else
                dist_sq += diff * diff;
        }
        total_dist += device_sqrt(dist_sq);
                                           ...
                                           T *__restrict__ eccentricities)
{
    ...
    for (int j = 0; j < n_points; ++j)
    {
        ...
        if constexpr (std::is_same_v<T, float>)
            max_dist_sq = ptx::hwmax_f32(max_dist_sq, dist_sq);
                             ...
                             T *__restrict__ cluster_sums, int *__restrict__ cluster_counts)
{
    ...
    for (int c = 0; c < k; ++c)
    {
        ...
        bool better = (dist_sq < best_dist_sq);
        if constexpr (std::is_same_v<T, float>)
        {
            best_dist_sq = ptx::slct_f32(better, dist_sq, best_dist_sq);
            best_cluster = ptx::slct_s32(better, c, best_cluster);
        }
        else if (better)
        {
            best_dist_sq = dist_sq;
                           ...
                           int *__restrict__ cover_indices, int max_cover_size)
{
    ...
    int n_intervals = resolution;
    int write_pos = 0;
    int total_sets = resolution;
    for (int d = 1; d < n_filter_dims; ++d)
        total_sets *= n_intervals;
    ...
    for (int s = 0; s < total_sets && write_pos < max_cover_size; ++s)
    {
        ...
        if (in_cover)
        {
            cover_indices[i * max_cover_size + write_pos] = s;
            ++write_pos;
        }
    }
    cover_sizes[i] = write_pos;
                                ...
                                int k_neighbors, std::function<void(std::vector<float>)> callback)
{
    ...
    if (is_sm80_or_newer())
        compute_density_filter_kernel_Ptx<float>
            <<<grid, kBlockSize>>>(d_points.get(), n_points, dim, k_neighbors, d_densities.get());
    else
        compute_density_filter_kernel_Orig<float>
            <<<grid, kBlockSize>>>(d_points.get(), n_points, dim, k_neighbors, d_densities.get());
                                     ...
                                     std::function<void(std::vector<float>)> callback)
{
    ...
    int grid = (n_points + kBlockSize - 1) / kBlockSize;
    if (is_sm80_or_newer())
        compute_eccentricity_filter_kernel_Ptx<float>
            <<<grid, kBlockSize>>>(d_points.get(), n_points, dim, d_eccentricities.get());
    else
        compute_eccentricity_filter_kernel_Orig<float>
            <<<grid, kBlockSize>>>(d_points.get(), n_points, dim, d_eccentricities.get());
    cudaDeviceSynchronize();
    cudaError_t launch_err = cudaGetLastError();
    if (launch_err != cudaSuccess)
