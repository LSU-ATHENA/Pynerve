...
public:
  GPUSheafLaplacian(int num_stalks, int max_stalk_dim)
      : num_stalks_(num_stalks), max_stalk_dim_(max_stalk_dim) {
  ...
  void buildLaplacian(const std::vector<int> &coboundary_row_ptr,
    ...
    const int restriction_count = checkedIntSize(restrictions.size(), "sheaf restriction count");
    ...
    for (size_t i = 1; i < coboundary_row_ptr.size(); ++i) {
      if (coboundary_row_ptr[i - 1] > coboundary_row_ptr[i] || coboundary_row_ptr[i] < 0) {
        throw std::invalid_argument("coboundary_row_ptr must be monotonic and non-negative");
      }
    }
    for (int col : coboundary_col_idx) {
      if (col < 0 || col >= num_stalks_) {
        throw std::out_of_range("coboundary_col_idx contains a stalk outside sheaf dimensions");
      }
    }
    ...
    auto free_runtime = [&]() {
      ...
      if (d_restrictions) cudaFree(d_restrictions);
    ...
    try {
      ...
      for (size_t r = 0; r < restrictions.size(); ++r) {
        ...
        if (map.from_stalk < 0 || map.from_stalk >= num_stalks_ || map.to_stalk < 0 ||
          ...
          throw std::invalid_argument("restriction map metadata is outside sheaf dimensions");
        }
        ...
        allocateDevice(&d_restriction_indices[r], nnz, "allocate sheaf restriction indices");
        allocateDevice(&d_restriction_values[r], nnz, "allocate sheaf restriction values");
        copyToDevice(d_restriction_indices[r], map.indices, nnz, "copy sheaf restriction indices");
        copyToDevice(d_restriction_values[r], map.values, nnz, "copy sheaf restriction values");
      ...
      const int blocks = checkedGridBlocks(static_cast<size_t>(num_stalks_),
                                           "sheaf Laplacian grid");
      sheafLaplacianKernel<<<blocks, BLOCK_SIZE>>>(
          d_row_ptr, d_col_idx, d_vals, d_restrictions, restriction_count,
      ...
      requireFiniteOutput(diagonal, "sheaf Laplacian construction produced non-finite values");
  ...
  void solve(const std::vector<float> &rhs, std::vector<float> &solution) {
    ...
    const int blocks = checkedGridBlocks(static_cast<size_t>(num_stalks_),
                                         "sheaf cohomology grid");
    sheafCohomologyKernel<<<blocks, BLOCK_SIZE>>>(d_laplacian_diag_, d_rhs_,
                                                  d_solution_, num_stalks_,
                                                  SHEAF_MAX_ITERATIONS);
  ...
  void applyRestriction(const std::vector<float> &from_data,
                        std::vector<float> &to_data,
    ...
    float *d_to = nullptr;
    auto free_runtime = [&]() {
      if (d_map_indices) cudaFree(d_map_indices);
      if (d_map_values) cudaFree(d_map_values);
    ...
    try {
      ...
      applyRestrictionKernel<<<blocks, BLOCK_SIZE>>>(d_from, d_to, d_map_indices, d_map_values,
                                                     map.nnz, from_dim, to_dim);
