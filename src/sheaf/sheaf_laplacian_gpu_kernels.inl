 ...
 */
__global__ void sheafLaplacianKernel(const int *__restrict__ coboundary_row_ptr,
                                     ...
                                     int num_restriction_maps, float *__restrict__ laplacian_values,
                                     int num_stalks)
{
 ...
 */
__global__ void stalkOperationKernel(float *__restrict__ stalk_data,
...
)
{
    ...
    switch (operation)
    {
        ...
        { // add
            ...
            stalk_data[idx] =
                (isfinite(stalk_data[idx]) && isfinite(input_data[idx]) && isfinite(next))
                    ? next
                    : INFINITY;
            break;
        }
 ...
 */
__global__ void applyRestrictionKernel(const float *__restrict__ from_stalk_data,
                                      ...
                                      int max_iters)
{
    ...
    const float diag = fabsf(laplacian_diag[idx]) > 1e-8f ? laplacian_diag[idx] : 1e-8f;
