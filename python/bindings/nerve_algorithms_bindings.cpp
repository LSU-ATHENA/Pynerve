...
template <typename T>
ValidatedArray<T> validate_point_array(CArray<T> array, size_t rows, size_t dim, const char *name)
{
    if (rows == 0)
    {
    ...
    {
        throw std::invalid_argument(std::string(name) + " must be a rank-1 flat or rank-2 array");
    }
    if (static_cast<size_t>(info.size) != expected)
    {
        throw std::invalid_argument(std::string(name) + " size must equal rows * dim");
    }
    if (info.ndim == 2 &&
        (static_cast<size_t>(info.shape[0]) != rows || static_cast<size_t>(info.shape[1]) != dim))
    {
        throw std::invalid_argument(std::string(name) + " shape must match rows and dim");
    }
    validate_finite_values<T>(info, name);
    return {std::move(array), std::span<const T>(static_cast<const T *>(info.ptr), expected)};
}

template <typename T>
ValidatedArray<T> validate_exact_array(CArray<T> array, size_t expected, const char *name)
{
    py::buffer_info info = array.request();
    if (static_cast<size_t>(info.size) != expected)
    {
        throw std::invalid_argument(std::string(name) + " size does not match expected layout");
    }
    validate_finite_values<T>(info, name);
    return {std::move(array), std::span<const T>(static_cast<const T *>(info.ptr), expected)};
}
...
ValidatedArray<T> validate_array_values(CArray<T> array, const char *name)
{
    ...
    return {std::move(array),
            std::span<const T>(static_cast<const T *>(info.ptr), static_cast<size_t>(info.size))};
}
...
void bind_distance_computer(py::module &m, const char *name)
{
    ...
    py::enum_<typename Config::Metric>(m, (std::string("DistanceMetric") + name).c_str())
    ...
    py::class_<DC>(m, name)
        ...
        .def(
            ...
            py::arg("points"), py::arg("n_points"), py::arg("dim"), "Compute full distance matrix")
        .def(
            "compute_pairwise",
            [](DC &self, CArray<T> set_a, size_t n_a, CArray<T> set_b, size_t n_b, size_t dim) {
                ...
                auto result =
                    self.compute_pairwise(parsed_a.values, n_a, parsed_b.values, n_b, dim);
                return copy_to_array(result, {n_a, n_b});
            },
            py::arg("set_a"), py::arg("n_a"), py::arg("set_b"), py::arg("n_b"), py::arg("dim"));
}
...
void bind_knn_computer(py::module &m, const char *name)
{
    ...
    py::enum_<typename Config::Algorithm>(m, (std::string("KNNAlgorithm") + name).c_str())
        .value("BRUTE_FORCE", Config::Algorithm::BRUTE_FORCE);

    py::enum_<typename Config::Metric>(m, (std::string("KNNMetric") + name).c_str())
        .value("EUCLIDEAN", Config::Metric::EUCLIDEAN)
        .value("MANHATTAN", Config::Metric::MANHATTAN)
        .value("COSINE", Config::Metric::COSINE);
    ...
    py::class_<KNN>(m, name)
        ...
        .def(
            ...
            py::arg("points"), py::arg("n_points"), py::arg("dim"), "Compute k-NN for all points")
        .def(
            "compute_query",
            [](KNN &self, CArray<T> reference, size_t n_ref, CArray<T> queries, size_t n_query,
               size_t dim) {
                auto parsed_ref =
                    validate_point_array(std::move(reference), n_ref, dim, "reference");
                auto parsed_queries =
                    validate_point_array(std::move(queries), n_query, dim, "queries");
                return self.compute_query(parsed_ref.values, n_ref, parsed_queries.values, n_query,
                                          dim);
            },
            py::arg("reference"), py::arg("n_ref"), py::arg("queries"), py::arg("n_query"),
            py::arg("dim"));
}
...
void bind_mapper(py::module &m, const char *name)
{
    ...
    py::class_<MA>(m, name)
        ...
        .def(
            "compute",
            [](MA &self, CArray<T> point_cloud, size_t n_points, size_t dim) {
                auto parsed =
    ...
    m.def(
        "connected_components", [](const Graph &graph) { return connected_components(graph); },
...
void bind_diagram_conv(py::module &m, const char *name)
{
    ...
    py::class_<Config>(m, (std::string("DiagramConvConfig") + name).c_str())
        ...
        .def_readwrite("use_persistence_weighting", &Config::use_persistence_weighting);

    py::class_<DC>(m, name)
        .def(py::init<Config>(), py::arg("config") = Config())
        .def(
            "forward",
            [](DC &self, CArray<T> diagram, CArray<T> features, size_t batch_size, size_t n_pairs) {
                if (batch_size == 0)
                {
                ...
                auto parsed_features = validate_array_values(std::move(features), "features");
                auto result = self.forward(parsed_diagram.values, parsed_features.values,
                                           batch_size, n_pairs);
                return copy_to_array(
                    result,
                    {batch_size, static_cast<size_t>(self.output_size(n_pairs) / batch_size)});
            },
            py::arg("diagram"), py::arg("features"), py::arg("batch_size"), py::arg("n_pairs"));
}
