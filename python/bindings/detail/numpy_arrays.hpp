...
template <typename T>
std::span<const T> to_span(py::array_t<T> array)
{
...
template <typename T>
py::array_t<T> copy_to_array(const T *data, size_t size, std::initializer_list<size_t> shape)
{
...
template <typename T>
py::array_t<T> copy_to_array(const std::vector<T> &values, std::initializer_list<size_t> shape)
{
