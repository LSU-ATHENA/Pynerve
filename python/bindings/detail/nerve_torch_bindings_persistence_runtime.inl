...
bool isSupportedTorchVrMetric(const std::string &metric) {
  return metric == "euclidean" || metric == "manhattan" ||
         metric == "chebyshev" || metric == "cosine";
}
...
VRPersistenceResult
vr_persistence_forward_impl(const at::Tensor &points, int64_t max_dim,
  ...
  TORCH_CHECK(points_proc.size(0) > 0,
              "points must contain at least one batch item");
  TORCH_CHECK(points_proc.size(1) > 0,
              "points must contain at least one point");
  TORCH_CHECK(points_proc.size(2) > 0,
              "points must contain at least one coordinate dimension");
  ...
  for (int64_t b = 0; b < batch_size; ++b) {
    ...
    auto persistence =
        compute_extended_persistence(dist_matrix, max_dim, max_radius);
    batch_persistence_info.push_back(persistence);

    // Convert to tensor format
    const auto n_pairs = static_cast<int64_t>(persistence.pairs.size());
    at::Tensor diagram = at::empty(
        {n_pairs, 3}, at::TensorOptions().dtype(at::kDouble).device(at::kCPU));
    auto diag_accessor = diagram.accessor<double, 2>();

    // Count pairs per dimension
    at::Tensor num_pairs = at::zeros(
        {max_dim + 1}, at::TensorOptions().dtype(at::kLong).device(at::kCPU));
    auto num_pairs_accessor = num_pairs.accessor<int64_t, 1>();

    for (int64_t pair_index = 0; pair_index < n_pairs; ++pair_index) {
      const auto [birth, death, dim] =
          persistence.pairs[static_cast<size_t>(pair_index)];
    ...
    at::Tensor mask = at::ones(
        {n_pairs}, at::TensorOptions().dtype(at::kBool).device(at::kCPU));
  ...
  at::TensorOptions bool_opts =
      at::TensorOptions().dtype(at::kBool).device(points.device());
  ...
  for (size_t i = 0; i < batch_diagrams.size(); ++i) {
    ...
    if (n_pairs < max_pairs) {
      ...
      at::Tensor padding =
          at::zeros({max_pairs - n_pairs, 3},
                    float_opts.dtype(at::kDouble).device(at::kCPU));
      at::Tensor padded_diag = at::cat({batch_diagrams[i], padding}, 0);
      padded_diagrams.push_back(padded_diag);

      // Pad mask with false
      at::Tensor mask_padding =
          at::zeros({max_pairs - n_pairs},
...
at::Tensor vr_persistence_backward_impl(const at::Tensor &grad_diagrams,
                                        const at::Tensor &points,
                                        const at::Tensor &birth_idx,
                                        const at::Tensor &death_idx) {
  ...
  if (points_cpu.dim() == 2) {
    const at::Tensor grad_pairs =
        (grad_cpu.dim() == 3) ? grad_cpu[0] : grad_cpu;
  ...
  TORCH_CHECK(grad_cpu.dim() == 3,
              "grad_diagrams must be rank-3 when points is batched");

  const int64_t batch_size = points_cpu.size(0);
  std::vector<ExtendedPersistenceResult> batched_info = load_persistence_info(points_cpu.data_ptr());
...
py::tuple vr_persistence_forward_py(const at::Tensor &points, int64_t max_dim,
                                    double max_radius,
                                    const std::string &metric,
                                    int64_t pi_resolution, double pi_sigma) {
  auto result = vr_persistence_forward_impl(points, max_dim, max_radius, metric,
                                            pi_resolution, pi_sigma);
