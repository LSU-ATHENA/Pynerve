void bind_functional_api(py::module &m) {
  m.def(
      "vr_persistence_forward", &vr_persistence_forward_py,
  ...
  m.def("vr_persistence_backward", &vr_persistence_backward_impl,
        "Compute gradients for VR persistence.", py::arg("grad_diagrams"),
        py::arg("points"), py::arg("birth_idx"), py::arg("death_idx"));
  ...
  m.def(
      ...
      [](const at::Tensor &distance_matrix, int64_t max_dim) {
        ...
        at::Tensor dims =
            at::zeros({n_pairs}, at::TensorOptions().dtype(at::kLong).device(
                                     distance_matrix.device()));
        at::Tensor mask =
            at::ones({n_pairs}, at::TensorOptions().dtype(at::kBool).device(
                                    distance_matrix.device()));
        at::Tensor num_pairs =
            at::zeros({max_dim + 1}, at::TensorOptions().dtype(at::kLong));
  ...
  m.def(
      ...
      [](const at::Tensor &diagram1, const at::Tensor &diagram2,
         double p) -> at::Tensor {
        using namespace nerve::torch;
      ...
      py::arg("diagram1"), py::arg("diagram2"), py::arg("p") = 2.0);
  ...
  m.def(
      ...
      py::arg("diagram1"), py::arg("diagram2"));

  // ML Representations

  // Persistence image
  m.def("ml_persistence_image", &ml_persistence_image,
        "Convert persistence diagram to image",
        py::arg("diagram"), py::arg("resolution_birth") = 20,
        py::arg("resolution_death") = 20, py::arg("sigma") = 0.5,
        py::arg("birth_min") = 0.0, py::arg("birth_max") = 0.0,
  ...
  m.def("ml_persistence_landscape", &ml_persistence_landscape,
        ...
        py::arg("diagram"), py::arg("k") = 5, py::arg("num_samples") = 100,
        py::arg("x_min") = 0.0, py::arg("x_max") = 0.0);

  // Persistence silhouette
  m.def("ml_persistence_silhouette", &ml_persistence_silhouette,
        "Convert persistence diagram to silhouette",
        py::arg("diagram"), py::arg("num_samples") = 100,
        py::arg("x_min") = 0.0, py::arg("x_max") = 0.0,
        py::arg("weight_fn") = "persistence");
  ...
  m.def(
      ...
      [](const at::Tensor &diagram, int64_t num_samples, double sigma,
        ...
        if (t_values.has_value()) {
          return ml_heat_kernel_signature(diagram, num_samples, sigma,
                                          t_values.value());
        }
  ...
  // Birth-death curve
  m.def("ml_birth_death_curve", &ml_birth_death_curve,
        "Compute birth-death histogram",
        py::arg("diagram"), py::arg("num_bins") = 50,
        py::arg("statistic") = "count");

  // ML Statistics

  // Total persistence
  m.def("ml_total_persistence", &ml_total_persistence,
        "Compute total persistence", py::arg("diagram"),
        py::arg("dim") = -1, py::arg("p") = 1.0);

  // Mean persistence
  m.def("ml_mean_persistence", &ml_mean_persistence,
        "Compute mean persistence", py::arg("diagram"),
        py::arg("dim") = -1);

  // Max persistence
  m.def("ml_max_persistence", &ml_max_persistence,
        "Compute maximum persistence", py::arg("diagram"),
        py::arg("dim") = -1);

  // Persistence variance
  m.def("ml_persistence_variance", &ml_persistence_variance,
        "Compute persistence variance", py::arg("diagram"),
        py::arg("dim") = -1);

  // Persistence entropy
  m.def("ml_persistence_entropy", &ml_persistence_entropy,
        "Compute persistence entropy", py::arg("diagram"),
        py::arg("dim") = -1, py::arg("base") = std::exp(1.0));

  // Number of features
  m.def("ml_number_of_features", &ml_number_of_features,
        "Count number of features", py::arg("diagram"),
        py::arg("dim") = -1, py::arg("min_persistence") = 0.0);

  // Betti curve
  m.def("ml_betti_curve", &ml_betti_curve,
        "Compute Betti curve", py::arg("diagram"),
        py::arg("num_samples") = 100, py::arg("dim") = -1);

  // Amplitude
  m.def("ml_amplitude", &ml_amplitude, "Compute diagram amplitude",
        py::arg("diagram"), py::arg("metric") = "persistence",
        py::arg("p") = 1.0, py::arg("dim") = -1);

  // Extract all features
  m.def("ml_extract_features", &ml_extract_features,
        "Extract all statistics as feature vector", py::arg("diagram"),
        py::arg("dims") = std::vector<int64_t>{0, 1});

  // ML Kernels

  // Gaussian kernel
  m.def("ml_gaussian_kernel", &ml_gaussian_kernel,
        "Compute Gaussian kernel between diagrams", py::arg("d1"),
        py::arg("d2"), py::arg("sigma") = 0.5,
        py::arg("distance_metric") = "euclidean");

  // Scale-space kernel
  m.def("ml_persistence_scale_space_kernel", &ml_persistence_scale_space_kernel,
        "Compute persistence scale-space kernel", py::arg("d1"),
        py::arg("d2"), py::arg("sigma") = 0.5, py::arg("weight") = 0.5);

  // Sliced Wasserstein kernel
  m.def("ml_sliced_wasserstein_kernel", &ml_sliced_wasserstein_kernel,
        "Compute sliced Wasserstein kernel", py::arg("d1"),
        py::arg("d2"), py::arg("num_slices") = 10, py::arg("sigma") = 0.5);

  // Fisher kernel
  m.def("ml_persistence_fisher_kernel", &ml_persistence_fisher_kernel,
        "Compute persistence Fisher kernel", py::arg("d1"),
        py::arg("d2"), py::arg("sigma") = 0.5);

  // Linear kernel
  m.def("ml_linear_kernel", &ml_linear_kernel,
        "Compute linear kernel via silhouette", py::arg("d1"),
        py::arg("d2"), py::arg("num_samples") = 100);

  // Compute kernel matrix
  m.def("ml_compute_kernel_matrix", &ml_compute_kernel_matrix,
        "Compute full kernel matrix", py::arg("diagrams"),
        py::arg("kernel") = "gaussian", py::arg("sigma") = 0.5,
        py::arg("num_slices") = 10);

  // Normalize kernel matrix
  m.def("ml_normalize_kernel_matrix", &ml_normalize_kernel_matrix,
        "Normalize kernel matrix", py::arg("K"));

  // Center kernel matrix
  m.def("ml_center_kernel_matrix", &ml_center_kernel_matrix,
        "Center kernel matrix", py::arg("K"));
  ...
  m.def("quick_mapper", &quick_mapper, "Quick mapper with default settings",
        py::arg("point_cloud"), py::arg("filter") = "pca_2d",
        py::arg("resolution") = 10, py::arg("overlap") = 0.25);

  // Filter functions
  m.def("filter_pca", &filter_pca, "PCA filter function for mapper",
        py::arg("point_cloud"), py::arg("n_components") = 2);

  m.def("filter_eccentricity", &filter_eccentricity,
        "Eccentricity filter function", py::arg("point_cloud"));
}
