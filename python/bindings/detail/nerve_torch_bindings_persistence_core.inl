...
struct UnionFind {
    ...
    explicit UnionFind(int64_t n) : parent(n), rank(n, 0), representative(n) {
...
static std::unordered_map<const void*, std::vector<ExtendedPersistenceResult>> g_persistence_info_map;
...
ExtendedPersistenceResult compute_extended_persistence(
    const at::Tensor& dist_matrix,
    int64_t max_dim,
    double max_radius) {
    ...
    for (const auto& edge : edges) {
        ...
        if (ru != rv) {
            ...
            cycle_candidates.push_back(CycleCandidate{
                .birth = edge.w,
    ...
    if (max_dim >= 1) {
        std::sort(cycle_candidates.begin(), cycle_candidates.end(), [](const CycleCandidate& lhs, const CycleCandidate& rhs) {
            ...
            for (int64_t i = 0; i < n; ++i) {
                for (int64_t j = i + 1; j < n; ++j) {
                    ...
                    for (int64_t k = j + 1; k < n; ++k) {
                        ...
                        triangle_deaths.push_back(std::max({ij_it->second, ik_it->second, jk_it->second}));
                    }
                }
            }
            ...
            for (int64_t j = i + 1; j < n && emitted < MAX_H2_PAIRS; ++j) {
                ...
                for (int64_t k = j + 1; k < n && emitted < MAX_H2_PAIRS; ++k) {
                    ...
                    for (int64_t l = k + 1; l < n && emitted < MAX_H2_PAIRS; ++l) {
                        ...
                        if (il_it == edge_weight.end() || jl_it == edge_weight.end() || kl_it == edge_weight.end()) {
                            continue;
                        }
                        const double birth = std::max({
                            ij_it->second, ik_it->second, il_it->second,
                            jk_it->second, jl_it->second, kl_it->second});
                        result.pairs.emplace_back(birth, std::numeric_limits<double>::infinity(), 2);
...
at::Tensor compute_vr_gradients(
    const at::Tensor& grad_diagrams,  // [n_pairs, 3] (birth_grad, death_grad, dim)
    ...
    const auto pair_count =
        std::min(static_cast<int64_t>(persistence.pairs.size()), grad_rows);
    for (int64_t pair_index = 0; pair_index < pair_count; ++pair_index) {
        ...
        if (dim >= 1 && birth_idx >= 0 && death_idx >= 0 &&
            birth_idx < n && death_idx < n && std::isfinite(death)) {
            double norm = 0.0;
            for (int64_t d = 0; d < point_dim; ++d) {
                const double delta = points[death_idx][d].item<double>() - points[birth_idx][d].item<double>();
                norm += delta * delta;
            }
