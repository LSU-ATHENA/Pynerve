...
{
    double s = 0.0;
    for (std::size_t i = 0; i < n; ++i) s += vec[i] * vec[i];
    return std::sqrt(s);
}
...
{
    double s = 0.0;
    for (std::size_t i = 0; i < n; ++i) { double d = a[i] - b[i]; s += d * d; }
    return s;
}

void abs(double *data, std::size_t n)
{
    for (std::size_t i = 0; i < n; ++i) data[i] = std::abs(data[i]);
}
...
{
    for (std::size_t i = 0; i < n; ++i) data[i] = std::sqrt(data[i]);
}
...
{
    for (std::size_t i = 0; i < n; ++i) data[i] = (data[i] < 0.0) ? 0.0 : data[i];
}
...
{
    double s = 0.0;
    for (std::size_t i = 0; i < dim; ++i) s += std::abs(a[i] - b[i]);
    return s;
}
...
void softmax(double *data, std::size_t n)
{
    if (n == 0) return;
    double mx = data[0];
    for (std::size_t i = 1; i < n; ++i) if (data[i] > mx) mx = data[i];
    double sum = 0.0;
    for (std::size_t i = 0; i < n; ++i) { data[i] = std::exp(data[i] - mx); sum += data[i]; }
    double inv = 1.0 / sum;
...
{
    for (std::size_t i = 0; i < n; ++i) data[i] = (data[i] < 0.0f) ? 0.0f : data[i];
}
...
void axpy_f16(nerve::simd::half alpha, const nerve::simd::half *x, nerve::simd::half *y, std::size_t n)
{
    float fa = nerve::simd::half_to_float(alpha);
    for (std::size_t i = 0; i < n; ++i)
        y[i] = nerve::simd::float_to_half(nerve::simd::half_to_float(y[i]) + fa * nerve::simd::half_to_float(x[i]));
}

void fmad_f16(const nerve::simd::half *a, const nerve::simd::half *b, nerve::simd::half *c, std::size_t n)
{
    for (std::size_t i = 0; i < n; ++i)
        c[i] = nerve::simd::float_to_half(nerve::simd::half_to_float(c[i]) + nerve::simd::half_to_float(a[i]) * nerve::simd::half_to_float(b[i]));
}
...
{
    for (std::size_t i = 0; i < n; ++i)
        data[i] = nerve::simd::float_to_half((nerve::simd::half_to_float(data[i]) - mean) * std_inv);
}
...
std::vector<double> make_rand_vec(std::size_t n, std::uint64_t seed, double lo = -10.0, double hi = 10.0)
{
...
{
    for (std::size_t i = 0; i < n; ++i)
        if (!approx_lax(simd[i], ref[i])) return false;
    return true;
}
...
{
    for (std::size_t i = 0; i < n; ++i)
        if (!approx(simd[i], ref[i])) return false;
    return true;
}
...
{
    const std::vector<std::size_t> sizes = {0, 1, 2, 3, 4, 5, 7, 8, 15, 16, 17, 31, 32, 33, 100, 1000};
    for (auto n : sizes)
    {
        ...
        {
            std::cerr << "  FAIL reduce_sum at n=" << n
                      << " simd=" << simd_val << " ref=" << ref_val << "\n";
            return false;
        }
    }
    return true;
...
bool test_reduce_max()
{
    ...
    for (auto n : sizes)
    {
        ...
        if (!approx(simd_val, ref_val))
        {
            std::cerr << "  FAIL reduce_max at n=" << n
                      << " simd=" << simd_val << " ref=" << ref_val << "\n";
            return false;
        }
    }
...
bool test_reduce_min()
{
    ...
    for (auto n : sizes)
    {
        ...
        if (!approx(simd_val, ref_val))
        {
            std::cerr << "  FAIL reduce_min at n=" << n
                      << " simd=" << simd_val << " ref=" << ref_val << "\n";
            return false;
        }
    }
...
bool test_sqdiff_sum()
{
    ...
    for (auto n : sizes)
    {
        ...
        if (!approx(simd_val, ref_val))
        {
            std::cerr << "  FAIL sqdiff_sum at n=" << n
                      << " simd=" << simd_val << " ref=" << ref_val << "\n";
            return false;
        }
    }
...
bool test_euclidean()
{
    ...
    for (auto dim : dims)
    {
        ...
        if (!approx(simd_val, ref_val))
        {
            std::cerr << "  FAIL euclidean at dim=" << dim
                      << " simd=" << simd_val << " ref=" << ref_val << "\n";
            return false;
        }
    }
...
bool test_manhattan()
{
    ...
    for (auto dim : dims)
    {
        ...
        if (!approx(simd_val, ref_val))
        {
            std::cerr << "  FAIL manhattan at dim=" << dim
                      << " simd=" << simd_val << " ref=" << ref_val << "\n";
            return false;
        }
    }
...
bool test_cosine()
{
    ...
    for (auto dim : dims)
    {
        ...
        if (!approx(simd_val, ref_val))
        {
            std::cerr << "  FAIL cosine at dim=" << dim
                      << " simd=" << simd_val << " ref=" << ref_val << "\n";
            return false;
        }
    }
    ...
    for (auto n : sizes)
    {
        for (auto mean : means)
        {
            for (auto si : std_invs)
            {
                ...
                if (!all_approx(data_simd.data(), data_ref.data(), n))
                {
                    std::cerr << "  FAIL batchnorm at n=" << n << " mean=" << mean << " std_inv=" << si << "\n";
                    return false;
                }
            }
        }
...
bool test_softmax()
{
    ...
    for (auto n : sizes)
    {
        ...
        // Verify sum ~= 1.0
        double sum = 0.0;
        for (std::size_t i = 0; i < n; ++i) sum += data_simd[i];
...
bool test_dequantize()
{
    ...
    for (auto n : sizes)
    {
        for (auto b : bits)
        {
            ...
            for (std::size_t i = 0; i < n; ++i)
                quant[i] = static_cast<std::uint8_t>(static_cast<std::uint64_t>(i * 7 + 13) % (static_cast<std::uint64_t>(scale) + 1));
...
bool test_zero_size_all_primitives()
{
    ...
    if (nerve::simd::simd_reduce_sum(dummy_a, 0) != 0.0) return false;
    if (nerve::simd::simd_dot(dummy_a, dummy_b, 0) != 0.0) return false;
    // norm2, sqdiff_sum on zero-size
    if (nerve::simd::simd_norm2(dummy_a, 0) != 0.0) return false;
    if (nerve::simd::simd_sqdiff_sum(dummy_a, dummy_b, 0) != 0.0) return false;
    ...
    if (nerve::simd::simd_euclidean(dummy_a, dummy_b, 0) != 0.0) return false;
    if (nerve::simd::simd_manhattan(dummy_a, dummy_b, 0) != 0.0) return false;
    // cosine(0-dim) -- dot=0, norm2=0 for both -> 1.0
    double cos0 = nerve::simd::simd_cosine(dummy_a, dummy_b, 0);
    if (std::abs(cos0 - 1.0) > 1e-15) return false;
...
bool test_add_f32()
{
    ...
    for (auto n : sizes)
    {
        ...
        for (std::size_t i = 0; i < n; ++i)
        {
            ...
            {
                std::cerr << "  FAIL add_f32 at n=" << n << " i=" << i
...
bool test_reduce_sum_f32()
{
    ...
    for (auto n : sizes)
    {
        ...
        if (std::abs(simd_val - ref_val) > 1e-4f * std::max(1.0f, std::max(std::abs(simd_val), std::abs(ref_val))))
        {
            std::cerr << "  FAIL reduce_sum_f32 at n=" << n
                      << " simd=" << simd_val << " ref=" << ref_val << "\n";
            return false;
        }
    }
...
bool test_sub_f32()
{
    ...
    for (auto n : sizes)
    {
        ...
        for (std::size_t i = 0; i < n; ++i)
        {
            if (std::abs(a_simd[i] - a_ref[i]) > 1e-6f)
            {
                std::cerr << "  FAIL sub_f32 at n=" << n << " i=" << i
                          << " simd=" << a_simd[i] << " ref=" << a_ref[i] << "\n";
                return false;
            }
        }
...
bool test_mul_f32()
{
    ...
    for (auto n : sizes)
    {
        ...
        for (std::size_t i = 0; i < n; ++i)
        {
            if (std::abs(a_simd[i] - a_ref[i]) > 1e-6f)
            {
                std::cerr << "  FAIL mul_f32 at n=" << n << " i=" << i
                          << " simd=" << a_simd[i] << " ref=" << a_ref[i] << "\n";
                return false;
            }
        }
...
bool test_reduce_max_f32()
{
    ...
    for (auto n : sizes)
    {
        ...
        {
            std::cerr << "  FAIL reduce_max_f32 at n=" << n
                      << " simd=" << simd_val << " ref=" << ref_val << "\n";
            return false;
        }
    }
...
bool test_norm2_f32()
{
    ...
    for (auto n : sizes)
    {
        ...
        if (std::abs(simd_val - ref_val) > 1e-6f)
        {
            std::cerr << "  FAIL norm2_f32 at n=" << n
                      << " simd=" << simd_val << " ref=" << ref_val << "\n";
            return false;
        }
    }
...
bool test_abs_f32()
{
    ...
    for (auto n : sizes)
    {
        ...
        for (std::size_t i = 0; i < n; ++i)
        {
            if (std::abs(data_simd[i] - data_ref[i]) > 1e-6f)
            {
                std::cerr << "  FAIL abs_f32 at n=" << n << " i=" << i
                          << " simd=" << data_simd[i] << " ref=" << data_ref[i] << "\n";
                return false;
            }
        }
...
bool test_relu_f32()
{
    ...
    for (auto n : sizes)
    {
        ...
        for (std::size_t i = 0; i < n; ++i)
        {
            if (std::abs(data_simd[i] - data_ref[i]) > 1e-6f)
            {
                std::cerr << "  FAIL relu_f32 at n=" << n << " i=" << i
                          << " simd=" << data_simd[i] << " ref=" << data_ref[i] << "\n";
                return false;
            }
        }
...
bool test_min_f32()
{
    ...
    for (auto n : sizes)
    {
        ...
        for (std::size_t i = 0; i < n; ++i)
        {
            if (std::abs(a_simd[i] - a_ref[i]) > 1e-6f)
            {
                std::cerr << "  FAIL min_f32 at n=" << n << " i=" << i
                          << " simd=" << a_simd[i] << " ref=" << a_ref[i] << "\n";
                return false;
            }
        }
...
bool test_max_f32()
{
    ...
    for (auto n : sizes)
    {
        ...
        for (std::size_t i = 0; i < n; ++i)
        {
            if (std::abs(a_simd[i] - a_ref[i]) > 1e-6f)
            {
                std::cerr << "  FAIL max_f32 at n=" << n << " i=" << i
                          << " simd=" << a_simd[i] << " ref=" << a_ref[i] << "\n";
                return false;
            }
        }
    ...
    for (auto n : sizes)
    {
        ...
        for (auto [lo, hi] : bounds)
        {
            ...
            for (std::size_t i = 0; i < n; ++i)
            {
                ...
                {
                    std::cerr << "  FAIL clamp_f32 at n=" << n << " lo=" << lo << " hi=" << hi << " i=" << i
                              << " simd=" << copy_simd[i] << " ref=" << copy_ref[i] << "\n";
                    return false;
                }
            }
        }
...
bool test_gemv_f32()
{
    ...
    for (auto [m, n] : dims)
    {
        ...
        for (std::size_t i = 0; i < m * n; ++i) A_simd[i] = A_ref[i] = static_cast<float>(A_d[i]);
        for (std::size_t i = 0; i < n; ++i) x[i] = static_cast<float>(x_d[i]);
        for (std::size_t i = 0; i < m; ++i) y_simd[i] = y_ref[i] = static_cast<float>(y_d[i]);
        float alpha = 1.5f;
        float beta  = 0.5f;
...
bool test_ger_f32()
{
    ...
    for (auto [m, n] : dims)
    {
        ...
        for (std::size_t i = 0; i < m * n; ++i) A_simd[i] = A_ref[i] = static_cast<float>(A_d[i]);
        for (std::size_t i = 0; i < m; ++i) x[i] = static_cast<float>(x_d[i]);
        for (std::size_t i = 0; i < n; ++i) y[i] = static_cast<float>(y_d[i]);
        float alpha = 2.0f;
...
bool test_sqdiff_sum_f32()
{
    ...
    for (auto dim : dims)
    {
        ...
        if (std::abs(simd_val - ref_val) > 1e-4f * std::max(1.0f, std::max(std::abs(simd_val), std::abs(ref_val))))
        {
            std::cerr << "  FAIL sqdiff_sum_f32 at dim=" << dim
                      << " simd=" << simd_val << " ref=" << ref_val << "\n";
            return false;
        }
    }
...
bool test_euclidean_f32()
{
    ...
    for (auto dim : dims)
    {
        ...
        if (std::abs(simd_val - ref_val) > 1e-6f)
        {
            std::cerr << "  FAIL euclidean_f32 at dim=" << dim
                      << " simd=" << simd_val << " ref=" << ref_val << "\n";
            return false;
        }
    }
...
bool test_cosine_f32()
{
    ...
    for (auto dim : dims)
    {
        ...
        if (std::abs(simd_val - ref_val) > 1e-6f)
        {
            std::cerr << "  FAIL cosine_f32 at dim=" << dim
                      << " simd=" << simd_val << " ref=" << ref_val << "\n";
            return false;
        }
    }
...
bool test_fmad_f32()
{
    ...
    for (auto n : sizes)
    {
        ...
        for (std::size_t i = 0; i < n; ++i)
        {
            ...
            {
                std::cerr << "  FAIL fmad_f32 at n=" << n << " i=" << i
                          << " simd=" << c_simd[i] << " ref=" << c_ref[i] << "\n";
                return false;
            }
        }
    }
...
bool test_reduce_min_f32()
{
    ...
    for (auto n : sizes)
    {
        ...
        if (std::abs(simd_val - ref_val) > 1e-6f)
        {
            std::cerr << "  FAIL reduce_min_f32 at n=" << n
                      << " simd=" << simd_val << " ref=" << ref_val << "\n";
            return false;
        }
    }
...
bool test_neg_f32()
{
    ...
    for (auto n : sizes)
    {
        ...
        for (std::size_t i = 0; i < n; ++i)
        {
            if (std::abs(data_simd[i] - data_ref[i]) > 1e-6f)
            {
                std::cerr << "  FAIL neg_f32 at n=" << n << " i=" << i
                          << " simd=" << data_simd[i] << " ref=" << data_ref[i] << "\n";
                return false;
            }
        }
...
bool test_sqrt_f32()
{
    ...
    for (auto n : sizes)
    {
        ...
        for (std::size_t i = 0; i < n; ++i)
        {
            if (std::abs(data_simd[i] - data_ref[i]) > 1e-6f)
            {
                std::cerr << "  FAIL sqrt_f32 at n=" << n << " i=" << i
                          << " simd=" << data_simd[i] << " ref=" << data_ref[i] << "\n";
                return false;
            }
        }
...
bool test_exp_f32()
{
    ...
    for (auto n : sizes)
    {
        ...
        for (std::size_t i = 0; i < n; ++i)
        {
            if (std::abs(data_simd[i] - data_ref[i]) > 1e-4f * std::max(1.0f, std::max(std::abs(data_simd[i]), std::abs(data_ref[i]))))
            {
                std::cerr << "  FAIL exp_f32 at n=" << n << " i=" << i
                          << " simd=" << data_simd[i] << " ref=" << data_ref[i] << "\n";
                return false;
            }
        }
...
bool test_log_f32()
{
    ...
    for (auto n : sizes)
    {
        ...
        for (std::size_t i = 0; i < n; ++i)
        {
            if (std::abs(data_simd[i] - data_ref[i]) > 1e-6f)
            {
                std::cerr << "  FAIL log_f32 at n=" << n << " i=" << i
                          << " simd=" << data_simd[i] << " ref=" << data_ref[i] << "\n";
                return false;
            }
        }
...
bool test_tanh_f32()
{
    ...
    for (auto n : sizes)
    {
        ...
        for (std::size_t i = 0; i < n; ++i)
        {
            if (std::abs(data_simd[i] - data_ref[i]) > 1e-6f)
            {
                std::cerr << "  FAIL tanh_f32 at n=" << n << " i=" << i
                          << " simd=" << data_simd[i] << " ref=" << data_ref[i] << "\n";
                return false;
            }
        }
...
bool test_add_f16()
{
    ...
    for (auto n : sizes)
    {
        ...
        for (std::size_t i = 0; i < n; ++i)
            a_ref[i] = nerve::simd::float_to_half(nerve::simd::half_to_float(a_ref[i]) + nerve::simd::half_to_float(b_ref[i]));
        for (std::size_t i = 0; i < n; ++i)
        {
            if (std::abs(nerve::simd::half_to_float(a_simd[i]) - nerve::simd::half_to_float(a_ref[i])) > 1e-3f)
            {
...
bool test_scale_f16()
{
    ...
    for (auto n : sizes)
    {
        for (auto alpha_f : alphas)
        {
            ...
            float fa = nerve::simd::half_to_float(alpha);
            for (std::size_t i = 0; i < n; ++i)
                data_ref[i] = nerve::simd::float_to_half(nerve::simd::half_to_float(data_ref[i]) * fa);
            for (std::size_t i = 0; i < n; ++i)
            {
                if (std::abs(nerve::simd::half_to_float(data_simd[i]) - nerve::simd::half_to_float(data_ref[i])) > 1e-3f)
                {
...
bool test_reduce_sum_f16()
{
    ...
    for (auto n : sizes)
    {
        ...
        {
            std::cerr << "  FAIL reduce_sum_f16 at n=" << n << " simd=" << simd_val << " ref=" << ref_sum << "\n";
            return false;
        }
    }
    return true;
...
bool test_abs_f16()
{
    ...
    for (auto n : sizes)
    {
        ...
        for (std::size_t i = 0; i < n; ++i)
            data_ref[i] = nerve::simd::float_to_half(std::abs(nerve::simd::half_to_float(data_ref[i])));
        for (std::size_t i = 0; i < n; ++i)
        {
            if (std::abs(nerve::simd::half_to_float(data_simd[i]) - nerve::simd::half_to_float(data_ref[i])) > 1e-3f)
            {
...
bool test_axpy_f16()
{
    ...
    for (auto n : sizes)
    {
        for (auto alpha_f : alphas)
        {
            ...
            for (std::size_t i = 0; i < n; ++i)
            {
                if (std::abs(nerve::simd::half_to_float(y_simd[i]) - nerve::simd::half_to_float(y_ref[i])) > 1e-3f)
                {
...
bool test_fmad_f16()
{
    ...
    for (auto n : sizes)
    {
        ...
        for (std::size_t i = 0; i < n; ++i)
        {
            if (std::abs(nerve::simd::half_to_float(c_simd[i]) - nerve::simd::half_to_float(c_ref[i])) > 1e-3f)
            {
...
bool test_clamp_f16()
{
    ...
    for (auto n : sizes)
    {
        ...
        for (auto [lo_f, hi_f] : bounds)
        {
            ...
            for (std::size_t i = 0; i < n; ++i)
            {
                if (std::abs(nerve::simd::half_to_float(copy_simd[i]) - nerve::simd::half_to_float(copy_ref[i])) > 1e-3f)
                {
                    std::cerr << "  FAIL clamp_f16 at n=" << n << " lo=" << lo_f << " hi=" << hi_f << " i=" << i << "\n";
                    return false;
                }
            }
        }
...
bool test_sqdiff_sum_f16()
{
    ...
    for (auto n : sizes)
    {
        ...
        {
            std::cerr << "  FAIL sqdiff_sum_f16 at n=" << n << " simd=" << simd_val << " ref=" << ref_sqdiff << "\n";
            return false;
        }
    }
    return true;
...
bool test_manhattan_f16()
{
    ...
    for (auto n : sizes)
    {
        ...
        {
            std::cerr << "  FAIL manhattan_f16 at n=" << n << " simd=" << simd_val << " ref=" << ref_manhattan << "\n";
            return false;
        }
    }
    return true;
...
bool test_euclidean_f16()
{
    ...
    for (auto n : sizes)
    {
        ...
        {
            std::cerr << "  FAIL euclidean_f16 at n=" << n << " simd=" << simd_val << " ref=" << ref_euclidean << "\n";
            return false;
        }
    }
    return true;
...
bool test_cosine_f16()
{
    ...
    for (auto n : sizes)
    {
        ...
        {
            std::cerr << "  FAIL cosine_f16 at n=" << n << " simd=" << simd_val << " ref=" << ref_cosine << "\n";
            return false;
        }
    }
    return true;
...
bool test_norm2_f16()
{
    ...
    for (auto n : sizes)
    {
        ...
        {
            std::cerr << "  FAIL norm2_f16 at n=" << n << " simd=" << simd_val << " ref=" << ref_norm2 << "\n";
            return false;
        }
    }
    return true;
...
bool test_dequantize_f16()
{
    ...
    for (auto n : sizes)
    {
        for (auto b : bits)
        {
            ...
            for (std::size_t i = 0; i < n; ++i)
            {
                if (std::abs(nerve::simd::half_to_float(deq_simd[i]) - nerve::simd::half_to_float(deq_ref[i])) > 1e-3f)
                {
                    std::cerr << "  FAIL dequantize_f16 at n=" << n << " bits=" << b << " i=" << i
                              << " simd=" << nerve::simd::half_to_float(deq_simd[i]) << " ref=" << nerve::simd::half_to_float(deq_ref[i]) << "\n";
                    return false;
                }
            }
        }
        ...
        for (auto mean : means)
        {
            for (auto si : std_invs)
            {
                ...
                for (std::size_t i = 0; i < n; ++i)
                {
                    if (std::abs(nerve::simd::half_to_float(data_simd[i]) - nerve::simd::half_to_float(data_ref[i])) > 1e-3f)
                    {
                        std::cerr << "  FAIL batchnorm_f16 at n=" << n << " mean=" << mean << " std_inv=" << si << "\n";
                        return false;
                    }
                }
            }
...
bool test_zero_size_f16()
{
    ...
    nerve::simd::simd_clamp_f16(dummy_a, nerve::simd::float_to_half(-1.0f), nerve::simd::float_to_half(1.0f), 0);
    ...
    if (nerve::simd::simd_reduce_sum_f16(dummy_a, 0) != 0.0f) return false;
    if (nerve::simd::simd_dot_f16(dummy_a, dummy_b, 0) != 0.0f) return false;
    if (nerve::simd::simd_sqdiff_sum_f16(dummy_a, dummy_b, 0) != 0.0f) return false;
    if (nerve::simd::simd_manhattan_f16(dummy_a, dummy_b, 0) != 0.0f) return false;
    if (nerve::simd::simd_euclidean_f16(dummy_a, dummy_b, 0) != 0.0f) return false;
    if (nerve::simd::simd_cosine_f16(dummy_a, dummy_b, 0) != 1.0f) return false;
    if (nerve::simd::simd_norm2_f16(dummy_a, 0) != 0.0f) return false;
    if (nerve::simd::simd_reduce_max_f16(dummy_a, 0) != 0.0f) return false;
    if (nerve::simd::simd_reduce_min_f16(dummy_a, 0) != 0.0f) return false;
    return true;
}
...
bool test_zero_size_f32()
{
    ...
    if (nerve::simd::simd_reduce_sum_f32(dummy_a, 0) != 0.0f) return false;
    if (nerve::simd::simd_reduce_max_f32(dummy_a, 0) != 0.0f) return false;
    if (nerve::simd::simd_reduce_min_f32(dummy_a, 0) != 0.0f) return false;
    if (nerve::simd::simd_dot_f32(dummy_a, dummy_b, 0) != 0.0f) return false;
    if (nerve::simd::simd_norm2_f32(dummy_a, 0) != 0.0f) return false;
    if (nerve::simd::simd_sqdiff_sum_f32(dummy_a, dummy_b, 0) != 0.0f) return false;
    if (nerve::simd::simd_euclidean_f32(dummy_a, dummy_b, 0) != 0.0f) return false;
    float cos0 = nerve::simd::simd_cosine_f32(dummy_a, dummy_b, 0);
    if (std::abs(cos0 - 1.0f) > 1e-6f) return false;
    return true;
}
...
int main()
{
    ...
    std::cout << "\n" << (failures == 0 ? "All tests passed." : "Some tests FAILED.")
              << " (" << (sizeof(tests)/sizeof(tests[0])) - failures << "/" << (sizeof(tests)/sizeof(tests[0])) << ")\n";
