...
int main() {
    ...
    // Test 1: Bottleneck distance between two small diagrams
    {
        ...
        double distance = nerve::metrics::bottleneck::adaptiveBottleneckDistance(
            diagram1, diagram2);
    ...
    {
        std::vector<std::pair<float, float>> diagram = {
            {0.0f, 0.5f},
            {0.1f, 0.3f}
        };
    ...
    // Test 3: Parallel bottleneck distances
    {
        ...
        std::vector<std::vector<std::pair<float, float>>> diagrams2 = {
            {{0.0f, 0.4f}, {0.15f, 0.35f}},
            {{0.05f, 0.7f}, {0.25f, 0.55f}}
        };

        auto distances = nerve::metrics::bottleneck::parallelBottleneckDistances(
            diagrams1, diagrams2);
    ...
    // Test 4: Sinkhorn (Wasserstein) distance between two diagrams
    {
        ...
        double distance = nerve::metrics::sinkhorn::sinkhornDiagramDistance(
            diagram1, diagram2, config);
        assert(std::isfinite(distance));
        assert(distance >= 0.0);
    }
    ...
    // Test 7: Sliced Wasserstein distance
    {
        ...
        double distance = nerve::metrics::sinkhorn::slicedWassersteinDistance(
            diagram1, diagram2, 50);
        assert(std::isfinite(distance));
        assert(distance >= 0.0);
    }
    ...
    // Test 8: Hierarchical Wasserstein
    {
        ...
        double distance = nerve::metrics::sinkhorn::hierarchicalWasserstein(
            diagram1, diagram2, 2);
        assert(std::isfinite(distance));
        assert(distance >= 0.0);
    }
