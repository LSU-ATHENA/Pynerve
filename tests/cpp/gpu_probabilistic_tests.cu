...
int main()
{
    if (!has_gpu())
    {
        std::cerr << "No CUDA device available -- skipping GPU probabilistic kernel coverage tests\n";
        return 0;
    }
    ...
    // Probabilistic: benchmarkProbabilistic
    {
        ...
        std::cout << "PASSED: benchmarkProbabilistic (100 samples, speedup="
                  << bench.speedup << ")\n";
    }
    ...
    // Probabilistic: benchmarkProbabilistic larger
    {
        ...
        assert(bench.cpu_time_ms >= 0.0);
        assert(bench.gpu_time_ms >= 0.0);
        std::cout << "PASSED: benchmarkProbabilistic (500 samples, speedup="
                  << bench.speedup << ")\n";
    }
