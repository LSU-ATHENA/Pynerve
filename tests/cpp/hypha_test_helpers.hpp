...
inline nerve::algebra::SimplicialComplex
build_vr_complex(const std::vector<std::vector<float>> &points, float threshold)
{
    ...
    for (std::size_t ei = 0; ei < edges.size(); ++ei)
    {
        ...
        for (std::size_t ej = ei + 1; ej < edges.size(); ++ej)
        {
            ...
            float da = std::sqrt((points[static_cast<std::size_t>(a)][0] -
                                  points[static_cast<std::size_t>(common)][0]) *
                                     (points[static_cast<std::size_t>(a)][0] -
                                      points[static_cast<std::size_t>(common)][0]) +
                                 (points[static_cast<std::size_t>(a)][1] -
                                  points[static_cast<std::size_t>(common)][1]) *
                                     (points[static_cast<std::size_t>(a)][1] -
                                      points[static_cast<std::size_t>(common)][1]) +
                                 (points[static_cast<std::size_t>(a)][2] -
                                  points[static_cast<std::size_t>(common)][2]) *
                                     (points[static_cast<std::size_t>(a)][2] -
                                      points[static_cast<std::size_t>(common)][2]));
            float db = std::sqrt((points[static_cast<std::size_t>(b)][0] -
                                  points[static_cast<std::size_t>(common)][0]) *
                                 ...
                                 (points[static_cast<std::size_t>(b)][1] -
                                  points[static_cast<std::size_t>(common)][1]) *
                                     (points[static_cast<std::size_t>(b)][1] -
                                      points[static_cast<std::size_t>(common)][1]) +
                                 (points[static_cast<std::size_t>(b)][2] -
                                  points[static_cast<std::size_t>(common)][2]) *
                                     (points[static_cast<std::size_t>(b)][2] -
                                      points[static_cast<std::size_t>(common)][2]));
...
inline std::vector<nerve::Pair>
reduce_sequential_fast(const std::vector<std::vector<int>> &boundary,
                       const std::vector<double> &filtration_values,
                       const std::vector<double> &row_filtration_values,
                       const std::vector<nerve::Dimension> &dims, int n_rows)
{
    ...
    for (int j = 0; j < n_cols; ++j)
    {
        ...
        nerve::Pair pair{};
        pair.dimension =
            static_cast<std::size_t>(j) < dims.size() ? dims[static_cast<std::size_t>(j)] : 0;
        std::size_t pu = static_cast<std::size_t>(p);
    ...
    for (const auto &[edge_pair_val, tri_list] : edge_to_triangles)
    {
        ...
        for (std::size_t ti = 0; ti < tri_list.size(); ++ti)
        {
            ...
            for (std::size_t tj = ti + 1; tj < tri_list.size(); ++tj)
            {
                ...
                std::array<int, 4> tet_key{tetra_verts[0], tetra_verts[1], tetra_verts[2],
                                           tetra_verts[3]};
                if (seen_tets.insert(tet_key).second)
                {
                    complex.addSimplexWithFiltration(
                        nerve::algebra::Simplex(
                            {tetra_verts[0], tetra_verts[1], tetra_verts[2], tetra_verts[3]}),
                        tetra_filt);
                }
            }
        }
