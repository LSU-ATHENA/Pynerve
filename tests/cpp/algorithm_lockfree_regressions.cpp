...
int main() {
...

  {
    ...
    const auto pairs =
        nerve::persistence::reduceMatrixLockfree(boundary, filtration, std::vector<nerve::Dimension>(kColumns, 1), 4);
  ...
  #if defined(NERVE_HAS_CUDA)
{
    ...
    assert(nerve::gpu::advanced::launchTileDistanceMatrix(
               &tile_point, &tile_distance, 1, 0, tile_config, nullptr) ==
    ...
    assert(kernel_dispatcher.computeDistanceMatrix(&tile_point, &tile_distance, 1, 0, 0.0) ==
           cudaErrorInvalidValue);
    assert(kernel_dispatcher.computeDistanceMatrix(
               &tile_point, &tile_distance, 1, 1,
               std::numeric_limits<double>::quiet_NaN()) == cudaErrorInvalidValue);
    assert(kernel_dispatcher.computeDistanceMatrix(
               &tile_point, &tile_distance, 1, 1,
               std::numeric_limits<double>::infinity()) == cudaErrorInvalidValue);
#if !HAS_CUDA
    assert(nerve::gpu::advanced::launchTileDistanceMatrix(
               &tile_point, &tile_distance, 1, 1, tile_config, nullptr) ==
           cudaErrorInvalidValue);
#endif
    ...
    assert(manager.computeDistanceMatrix(invalid_gpu_manager_points, distances)
               .isError());
    assert(distances.empty());
    const std::vector<std::vector<double>> overflowing_gpu_manager_points{
        {0.0}, {std::numeric_limits<double>::max()}};
    assert(manager.computeDistanceMatrix(overflowing_gpu_manager_points, distances)
               .isError());
    assert(distances.empty());
    ...
    assert(manager.constructLaplacian(points, {{0, 1}, {1, 2}}, laplacian)
               .isSuccess());
    ...
    const auto invalid_clearing =
        manager.applyClearing(empty_clearing_boundary, empty_simplex_dimensions,
                              empty_filtration_values, 0,
                              std::numeric_limits<double>::infinity(),
                              clearing_result);
    assert(invalid_clearing.isError());
    assert(invalid_clearing.errorCode() ==
           nerve::errors::ErrorCode::E51_PH_INPUT);

    nerve::persistence::accelerated::EdgeExtractionConfig edge_config;
    edge_config.max_edges = 4;
    auto edge_extractor =
        nerve::persistence::accelerated::CUDAEgdeExtractor::create(
            edge_config);
    assert(edge_extractor.isSuccess());
    std::vector<double> invalid_distances{
        0.0, std::numeric_limits<double>::infinity(),
        std::numeric_limits<double>::infinity(), 0.0};
    std::vector<nerve::persistence::accelerated::Edge> extracted_edges(4);
    nerve::core::BufferView<const double> invalid_distance_view(
        invalid_distances.data(), invalid_distances.size());
    nerve::core::BufferView<nerve::persistence::accelerated::Edge>
        edge_view(extracted_edges.data(), extracted_edges.size());
    const auto invalid_edge_extraction =
        edge_extractor.value()->extractEdges(invalid_distance_view,
                                             std::move(edge_view), 2, 2.0);
    assert(invalid_edge_extraction.isError());
    assert(invalid_edge_extraction.errorCode() ==
           nerve::errors::ErrorCode::E20_NUM_NAN);
    ...
    assert(manager
               .buildCechComplex(large_cech_points, large_cech_coord, 2,
                                 cech_simplices)
               .isSuccess());
    assert(std::ranges::all_of(cech_simplices, [](const auto &simplex) {
      return std::isfinite(simplex.filtration_value) &&
             std::isfinite(simplex.alpha_value);
    }));
    assert(std::ranges::any_of(cech_simplices, [](const auto &simplex) {
      return simplex.dimension == 2;
    }));
    ...
    assert(manager
               .computeDiagramCostMatrix(diagram_cost_a, diagram_cost_b,
                                         diagram_cost_matrix)
               .isSuccess());
    ...
    overflow_diagram_cost.addPair({-std::numeric_limits<double>::max(),
    ...
    assert(manager
               .computeDiagramCostMatrix(negative_dimension_diagram_cost,
                                         diagram_cost_a, diagram_cost_matrix)
               .isError());
    assert(diagram_cost_matrix.empty());
    ...
    assert(manager
               .detectAffectedRegion(overflowing_streaming_window,
                                     overflowing_streaming_slide, affected)
               .isError());
    assert(affected.empty());
