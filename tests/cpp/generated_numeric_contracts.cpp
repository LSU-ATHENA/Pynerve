...
double value_at(const Case& test_case, int row, int col) {
    ...
    state ^= static_cast<std::uint64_t>(test_case.topological_dimension + 41) * 1609587929392839161ULL;
...
bool check_case(const Case& test_case) {
    ...
    return std::all_of(row_norms.begin(), row_norms.end(), [](double norm) {
...
bool check_knn_algorithm_contract() {
    ...
    if (!std::all_of(result.distances.begin(), result.distances.end(), [](double value) {
...
bool check_distance_allocation_overflow_contract() {
    ...
    if (!throws_invalid_argument([&]() { return distance.compute_pairwise(empty, huge, empty, 2, 0); })) {
        return false;
    }
...
bool check_distance_c_api_boundary_contract() {
    ...
    if (nerve_pairwise_distances_f64_status(points, 1, 1, nullptr) !=
        NERVE_STATUS_INVALID_ARGUMENT) {
        return false;
    }
    ...
    if (nerve_knn_f64_status(nullptr, 2, 1, 1, distances, indices) !=
        NERVE_STATUS_INVALID_ARGUMENT) {
        return false;
    }
    if (nerve_knn_f64_status(points, 2, 1, 1, nullptr, indices) !=
        NERVE_STATUS_INVALID_ARGUMENT) {
        return false;
    }
...
int main() {
    ...
    const std::vector<std::pair<int, int>> shapes = {{1, 2}, {2, 3}, {4, 4}, {8, 3}, {16, 8}, {32, 16}};
                ...
                for (const auto& scenario : scenarios) {
                    for (int seed = 0; seed < 4; ++seed) {
                        ...
                        if (!check_case(test_case) || !check_gradient(seed)) {
                            std::cerr << "generated numeric contract failed for " << op << " " << dtype
                                      << " dim=" << scenario.topological_dimension
