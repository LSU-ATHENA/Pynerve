...
#include "nerve/interaction/interaction_homology.hpp"
...
{
nerve::errors::ErrorResult<void> validateMetrics(const nerve::common::AcceleratedPerformanceStats &)
{
    return nerve::errors::ErrorResult<void>::error(nerve::errors::ErrorCode::E54_PH4_INVALID_INPUT,
                                                   "noop");
}
...
{
    {
        ...
        assert(metric.compute(std::span<const long double>(long_double_points.data(), 2),
                              std::span<const long double>(long_double_points.data() + 2, 2)) ==
               5.0L);
        ...
        simd_distance.computeBatch(simd_points.data(), 2, 2, {{0, 1}}, simd_results);
        ...
        try
        {
            simd_distance.computeBatch(simd_points.data(), 2, 2, {{0, 2}}, simd_results);
        }
        ...
        try
        {
            ...
            simd_distance.computeBatch(invalid_simd_points.data(), 2, 2, {{0, 1}}, simd_results);
        }
...

    {
        ...
        const auto cert = CertificateFactory::createPh5Ph6Certificate(
            ...
            std::numeric_limits<float>::infinity(), std::numeric_limits<float>::quiet_NaN(), 6, 1,
            std::numeric_limits<float>::infinity(), std::numeric_limits<float>::quiet_NaN());
        assert(cert.isValid());
...

    {
        ...
        Cycle cycle{{0, 1}, {1.0, -1.0}, 1, 0.0, std::numeric_limits<double>::infinity()};
...

    {
        ...
        MatrixCycle cycle{{0, 1}, {1.0, -1.0}, 1, 0.0, std::numeric_limits<double>::infinity()};
        assert(cycle.isValid());
...

    {
        ...
        try
        {
            (void)getOptimalRoaringConfig(2048, std::numeric_limits<double>::quiet_NaN());
        }
        catch (const std::invalid_argument &)
        {
...

    {
        ...
        try
        {
            const std::vector<double> bad_filter{0.0, std::numeric_limits<double>::quiet_NaN(),
                                                 1.0};
        ...
        try
        {
            const std::vector<double> bad_points{0.0, 0.0, std::numeric_limits<double>::quiet_NaN(),
                                                 1.0};
        ...
        try
        {
            ...
            const std::vector<double> bad_points{0.0, 0.0, 1.0,
        ...
        assert(pca.apply(tiny_span, std::numeric_limits<std::size_t>::max(), 2).empty());

        DBSCANClustering<double>::Config dbscan_config;
        DBSCANClustering<double> dbscan(dbscan_config);
        assert(dbscan.cluster(tiny_span, std::numeric_limits<std::size_t>::max(), 1).empty());
