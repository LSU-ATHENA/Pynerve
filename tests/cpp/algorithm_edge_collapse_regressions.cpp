...
int main() {
  {
    ...
    assert(!high_dim_defaults.use_gpu_acceleration);
    const auto high_dim_optimal = getOptimalHighDimensionalConfig(50000, 6);
    assert(!high_dim_optimal.use_gpu_acceleration);
    const auto empty_high_dim_benchmark =
        benchmarkHighDimensional({}, {}, {}, 6);
    ...
    ::nerve::core::BufferView<const double> api_view(api_points.data(),
                                                       api_points.size());
    ...
    nerve::persistence::adaptive_acceleration::ProblemCharacteristics
        adaptive_problem;
    adaptive_problem.estimated_columns = 100000;
    adaptive_problem.memory_requirement_mb = 1.0;
    nerve::persistence::adaptive_acceleration::SystemCapabilities
        adaptive_system;
    adaptive_system.cuda_available = true;
    adaptive_system.available_memory = 64ULL * 1024ULL * 1024ULL * 1024ULL;
    assert(!nerve::persistence::adaptive_acceleration::ProblemAnalyzer::
               shouldUseGpu(adaptive_problem, adaptive_system));
    ...
    const auto malformed_adaptive =
        nerve::persistence::adaptive_acceleration::ProblemAnalyzer::
            analyzeProblem(malformed_adaptive_view, 2);
    ...
    const auto nonfinite_adaptive =
        nerve::persistence::adaptive_acceleration::ProblemAnalyzer::
            analyzeProblem(nonfinite_adaptive_view, 2);
    ...
    adaptive_problem.memory_requirement_mb =
        std::numeric_limits<double>::infinity();
    assert(std::isfinite(
        nerve::persistence::adaptive_acceleration::ProblemAnalyzer::
            estimateComplexity(adaptive_problem)));
    assert(std::isfinite(
        nerve::persistence::adaptive_acceleration::ProblemAnalyzer::
            estimateMemoryRequirement(adaptive_problem)));
    assert(std::isfinite(
        nerve::persistence::adaptive_acceleration::ProblemAnalyzer::
            estimateApparentPairRatio(adaptive_problem)));
    const auto adaptive_predictions =
        nerve::persistence::adaptive_acceleration::PerformancePredictor::
            predictPerformance(adaptive_problem, adaptive_system);
    ...
    assert(SparseMatrix::fromBoundaryMatrix(
               {}, static_cast<size_t>(std::numeric_limits<int>::max()) + 1, 0)
               .isError());
    const auto sparse_ok =
        SparseMatrix::fromDenseMatrix({1.0, 0.0, 0.0, 2.0}, 2, 2);
    ...
    try {
      (void)collapseEdges(neighbors, edge_weights,
