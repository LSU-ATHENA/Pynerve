...
int main()
{
    ...
    // Mapper: density filter kernel smoke
    {
        ...
        nerve::gpu::mapper::compute_density_filter_gpu(
            points, 4, 3, 2, [&](std::vector<float> result) {
                assert(!result.empty());
                called = true;
            });
        assert(called);
    ...
    // Mapper: eccentricity filter kernel smoke
    {
        ...
        for (int i = 0; i < 8; ++i)
            points[i] = static_cast<float>(i);
        bool called = false;
        nerve::gpu::mapper::compute_eccentricity_filter_gpu(
            points, 4, 2, [&](std::vector<float> result) {
                assert(!result.empty());
                called = true;
            });
        assert(called);
    ...
    // Mapper: kmeans clustering kernel smoke
    {
        ...
        for (int i = 0; i < 8; ++i)
            points[i] = static_cast<float>(i);
        bool called = false;
        nerve::gpu::mapper::compute_kmeans_clustering_gpu(
            points, 4, 2, 2, 10, [&](std::vector<int> result) {
                assert(!result.empty());
                called = true;
            });
        assert(called);
