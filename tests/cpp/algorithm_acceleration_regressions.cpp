...
int main() {
  {
    ...
    assert(acceleration_defaults.mode ==
           nerve::common::AccelerationMode::CPU_ONLY);
    ...
    assert(metrics_defaults.execution_mode ==
           nerve::common::ExecutionMode::CPU_ONLY);
    assert(metrics_defaults.gpu_work_ratio == 0.0);

    nerve::common::VRConfig fast_vr_defaults;
    assert(fast_vr_defaults.acceleration.mode ==
           nerve::common::AccelerationMode::CPU_ONLY);
    ...
    cuda_capabilities.cuda_available = true;
        ...
        // relaxed
        // relaxed
    assert(!nerve::persistence::accelerated::utils::isAccelerationBeneficial(
        50000, 32, 1.0));

    const auto runtime_optimal =
        nerve::persistence::accelerated::utils::createOptimalConfigForProblem(
            50000, 32, 1.0, fast_vr_defaults);
    assert(runtime_optimal.acceleration.mode ==
           nerve::common::AccelerationMode::CPU_ONLY);
    ...
    const auto runtime_estimate =
        nerve::persistence::accelerated::utils::estimatePerformance(
    ...
    invalid_runtime_config.max_radius =
        std::numeric_limits<double>::quiet_NaN();
    assert(invalid_runtime_config.validate().isError());
    assert(nerve::persistence::accelerated::utils::
               validateAccelerationConfig(invalid_runtime_config)
                   .isError());

    bool rejected_runtime_nan_radius = false;
    try {
      (void)nerve::persistence::accelerated::utils::
          createOptimalConfigForProblem(
              50000, 32, std::numeric_limits<double>::quiet_NaN(),
              fast_vr_defaults);
    ...
    auto distributor =
        nerve::persistence::accelerated::WorkDistributor::create();
    assert(distributor.isSuccess());
    const auto default_distribution =
        distributor.value()->computeDistribution(10000);
    ...
    nerve::persistence::accelerated::WorkDistributor::Config
        explicit_gpu_split;
    ...
    const auto explicit_distribution =
        explicit_distributor.value()->computeDistribution(1000);
    ...
    assert(medium_fast_config.algorithm ==
    ...
    ::nerve::core::BufferView<const double> view(points.data(),
                                                   points.size());
    auto invalid_accelerated_call_config = fast_vr_defaults;
    invalid_accelerated_call_config.max_radius =
        std::numeric_limits<double>::quiet_NaN();
    assert(nerve::persistence::accelerated::computeVrPersistenceFast(
               view, 2, invalid_accelerated_call_config)
               .isError());

    nerve::persistence::accelerated::GPUApparentPairs::Config
        invalid_apparent_config;
    invalid_apparent_config.gpu_work_ratio =
        std::numeric_limits<double>::quiet_NaN();
    assert(invalid_apparent_config.validate().isError());
    assert(nerve::persistence::accelerated::GPUApparentPairs::create(
               invalid_apparent_config)
               .isError());
    ...
    const auto hetero_pairs =
        hetero_engine.value()->computeVrPersistence(points, 2);
    ...
    assert(nerve::gpu::HeterogeneousEngine::create(gpu_requested_hetero)
               .isErr());
    ...
    assert(hetero_engine.value()
               ->computeVrPersistence(invalid_hetero_points, 2)
               .isErr());
    ...
    try {
      ...
      (void)computeSparseRips(nonfinite_sparse_points, 2, 2,
                              sparse_config_valid);
    } catch (const std::invalid_argument &) {
    ...
    const auto invalid_savings = estimateSparseRipsSavings(
    ...
    assert(
        !shouldUseSparseRips(1000, std::numeric_limits<double>::quiet_NaN()));
    ...
    try {
      auto invalid_sketch = sketch_config;
      invalid_sketch.edge_sampling_rate =
    ...
    try {
      ...
      (void)computeApproximatePHSketching(nonfinite_sketch_view, 2,
                                          sketch_config);
    } catch (const std::invalid_argument &) {
    ...
    assert(LandmarkSelector::selectLandmarks({}, 2, 0, 3,
    ...
    const auto landmarks = LandmarkSelector::selectLandmarks(
        points, 2, 3, 10, LandmarkSelector::Strategy::MAXMIN);
...

    {
      ...
      const double cpu_estimate =
          nerve::persistence::accelerated::utils::estimateComputationTime(
              2000, 2, 1.0, false);
      ...
      const auto cpu_memory =
          nerve::persistence::accelerated::utils::estimateMemoryUsage(
              2000, 2, 1.0, false);
      ...
      const auto cuda_memory =
          nerve::persistence::accelerated::utils::estimateTotalGpuMemoryUsage(
              3, 2, 2.0, 0.5);
      ...
      try {
        (void)nerve::persistence::accelerated::utils::
            estimateEdgeExtractionMemoryUsage(
      ...
      try {
        (void)nerve::persistence::accelerated::utils::
            estimateMatrixReductionMemoryUsage(3, 2, 1.5);
      } catch (const std::invalid_argument &) {
      ...
      nerve::common::PerformanceMetrics invalid_impact_metric =
          finite_detail_metric;
      invalid_impact_metric.total_time_ms =
          std::numeric_limits<double>::quiet_NaN();
      invalid_impact_metric.gpu_bytes =
          std::numeric_limits<double>::quiet_NaN();
      assert(nerve::persistence::accelerated::performance_impact::
                 computeRuntimeChange(invalid_impact_metric,
                                      finite_detail_metric) == 0.0);
      assert(nerve::persistence::accelerated::performance_impact::
                 computeMemoryChange(invalid_impact_metric,
                                     finite_detail_metric) == 0.0);
      assert(std::isfinite(nerve::persistence::accelerated::
                               performance_impact::computeOverallImpactScore(
                                   invalid_impact_metric, finite_detail_metric)));
      const auto invalid_tuning_advice =
          nerve::persistence::accelerated::optimization_recommendations::
              suggestActions(invalid_impact_metric);
      assert(invalid_tuning_advice.size() == 1);
      assert(invalid_tuning_advice.front().find("finite") !=
             std::string::npos);

      nerve::persistence::accelerated::PerformanceMonitor monitor;
      nerve::common::PerformanceMetrics invalid_recorded_metric{};
      invalid_recorded_metric.total_time_ms =
          std::numeric_limits<double>::quiet_NaN();
      invalid_recorded_metric.cpu_time_ms =
          std::numeric_limits<double>::quiet_NaN();
      invalid_recorded_metric.gpu_time_ms = 1.0;
      invalid_recorded_metric.max_radius =
          std::numeric_limits<double>::quiet_NaN();
      invalid_recorded_metric.gpu_work_ratio =
          std::numeric_limits<double>::quiet_NaN();
      invalid_recorded_metric.problem_ops =
          std::numeric_limits<double>::infinity();
      invalid_recorded_metric.gpu_bytes =
          std::numeric_limits<double>::infinity();
      invalid_recorded_metric.gpu_stage_ops =
          std::numeric_limits<double>::infinity();
      ...
      assert(nerve::persistence::accelerated::accelerated_error_tools::
                 validateMetrics(invalid_perf_stats)
                     .isError());

      invalid_perf_stats.speedup = 1.0;
      invalid_perf_stats.memory_usage_mb =
          std::numeric_limits<double>::infinity();
      assert(invalid_perf_stats.getOverallEfficiency() == 0.0);
      assert(nerve::persistence::accelerated::accelerated_error_tools::
                 validateMetrics(invalid_perf_stats)
      ...
      assert(nerve::persistence::accelerated::accelerated_error_tools::
                 validatePairs(invalid_accel_pairs)
                     .isError());

      const std::vector<nerve::persistence::Pair>
          negative_dimension_accel_pairs{{0.0, 1.0, -1}};
      assert(nerve::persistence::accelerated::accelerated_error_tools::
                 validatePairs(negative_dimension_accel_pairs)
                     .isError());
      ...
      assert(nerve_interface.value()
                 ->computeVrPersistence(nonfinite_accel_view, 2)
                 .isError());
    }
  }
...

  {
    ...
    const nerve::PointCloud invalid_witness_points{
        {0.0, 0.0}, {std::numeric_limits<double>::quiet_NaN(), 1.0}};
    const auto invalid_ph6 =
        ph6.computePersistenceWitness(invalid_witness_points, 2, 0.5);
...

  {
    ...
    const auto result =
        nerve::persistence::specialized::computeDimensionSpecialized(
    ...
    const auto recommended =
        nerve::persistence::specialized::getOptimalDimensionConfig(60000, 3,
                                                                     50000);
    ...
    const auto estimate =
        nerve::persistence::specialized::estimateDimensionSpeedup(3, 60000,
                                                                    50000);
...

  {
    ...
    const auto result =
        nerve::persistence::h2::computeH2AlphaComplex(points, 2, 3, config);
...

  {
    ...
    const auto h1_optimal =
        nerve::persistence::h1::getOptimalH1Config(50000, 2.0);
    ...
    const auto h2_optimal =
        nerve::persistence::h2::getOptimalH2Config(50000, 2, 2.0);
