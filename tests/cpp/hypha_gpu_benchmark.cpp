...
BenchmarkResult run_benchmark(int num_points, float threshold, unsigned seed, int dim)
{
    ...
    auto _ = nerve::persistence::reduceMatrixLockfree(lockfree_boundary, lockfree_filtration,
                                                      &lockfree_row_filtration, lockfree_dims,
    ...
    if (!phase_samples.empty())
    {
        auto median_phase = [&](auto getter) -> double {
            ...
            for (auto &s : phase_samples)
                vals.push_back(getter(s));
            std::sort(vals.begin(), vals.end());
            return vals[vals.size() / 2];
        };
        result.phases.csc_build_ms = median_phase([](const auto &s) { return s.csc_build_ms; });
        result.phases.clearing_ms = median_phase([](const auto &s) { return s.clearing_ms; });
        result.phases.submatrix_build_ms =
            median_phase([](const auto &s) { return s.submatrix_build_ms; });
        result.phases.gpu_pack_ms = median_phase([](const auto &s) { return s.gpu_pack_ms; });
        ...
        result.phases.overhead_ms = median_phase([](const auto &s) { return s.overhead_ms; });
    }
    ...
    for (int t = 0; t < kTrials; ++t)
    {
        ...
        double ms = std::chrono::duration<double, std::milli>(end - start).count();
        if (t > 0)
            lockfree_times.push_back(ms);
        if (t == kTrials - 1)
        {
            result.lockfree_pair_count = static_cast<int>(pairs.size());
            result.pairs_match =
                result.pairs_match && (result.hypha_pair_count == static_cast<int>(pairs.size()));
    ...
    result.speedup =
        (result.lockfree_time_ms > 0.0) ? result.lockfree_time_ms / result.hypha_time_ms : 1.0;
    return result;
}
...
int main()
{
    ...
    auto print_header = [&](int dim) {
        if (dim == 3)
        {
            std::snprintf(
                buf, sizeof(buf),
                "  %-6s | %-11s | %-7s | %-11s | %-12s | %-11s | %-14s | %-9s | %-6s | %s\n",
                "Points", "Simplices", "Edges", "Triangles", "Tetrahedra", "Hypha(ms)",
                "Lockfree(ms)", "Speedup", "Cols", "OK");
            write(buf);
            std::snprintf(buf, sizeof(buf), "  %s\n",
                          "-------+-------------+---------+-------------+--------------+-----------"
                          "--+----------------+-----------+--------+-----");
        ...
        else
        {
            std::snprintf(buf, sizeof(buf),
                          "  %-6s | %-11s | %-7s | %-11s | %-11s | %-14s | %-9s | %-6s | %s\n",
                          "Points", "Simplices", "Edges", "Triangles", "Hypha(ms)", "Lockfree(ms)",
                          "Speedup", "Pairs", "OK");
            write(buf);
            std::snprintf(buf, sizeof(buf), "  %s\n",
                          "-------+-------------+---------+-------------+-------------+------------"
    ...
    BenchConfig configs[] = {
        {20, 0.8f, 42, 2},   {50, 0.6f, 42, 2},   {100, 0.5f, 42, 2},   {200, 0.40f, 42, 2},
        {500, 0.28f, 42, 2}, {800, 0.20f, 42, 2}, {1200, 0.15f, 42, 2}, {2000, 0.10f, 42, 2},
        {10, 1.0f, 42, 3},   {15, 0.9f, 42, 3},   {20, 0.8f, 42, 3},    {30, 0.7f, 42, 3},
        {50, 0.5f, 42, 3},
    };
    ...
    for (const auto &cfg : configs)
    {
        ...
        if (result.lockfree_time_ms > 0.0)
        {
            if (cfg.dim == 3)
            {
                std::snprintf(
                    ...
                    result.num_points, static_cast<std::size_t>(result.num_simplices),
                    static_cast<std::size_t>(result.num_edges),
                    static_cast<std::size_t>(result.num_triangles),
                    static_cast<std::size_t>(result.num_tetrahedra), result.hypha_time_ms,
                    result.lockfree_time_ms, result.speedup,
            ...
            else
            {
                std::snprintf(
                    ...
                    result.num_points, static_cast<std::size_t>(result.num_simplices),
                    static_cast<std::size_t>(result.num_edges),
                    static_cast<std::size_t>(result.num_triangles), result.hypha_time_ms,
                    result.lockfree_time_ms, result.speedup, result.hypha_pair_count,
                    result.lockfree_pair_count, result.pairs_match ? "OK" : "MISMATCH");
            }
        }
        else
        {
            if (cfg.dim == 3)
            {
                std::snprintf(
                    ...
                    static_cast<std::size_t>(result.num_edges),
                    static_cast<std::size_t>(result.num_triangles),
                    static_cast<std::size_t>(result.num_tetrahedra), "N/A", "N/A", "N/A", "N/A",
                    result.pairs_match ? "OK" : "MISMATCH");
            }
            else
            {
                std::snprintf(buf, sizeof(buf),
                              ...
                              result.num_points, static_cast<std::size_t>(result.num_simplices),
                              static_cast<std::size_t>(result.num_edges),
                              static_cast<std::size_t>(result.num_triangles), "N/A", "N/A", "N/A",
                              "N/A", result.pairs_match ? "OK" : "MISMATCH");
            }
        }
        write(buf);
        ...
        if (p.csc_build_ms > 0.0 || p.gpu_reduction_ms > 0.0)
        {
            std::snprintf(
                buf, sizeof(buf),
                "  %46s  %9s: %8.2f ms | clearing: %8.2f ms | submatrix: %8.2f ms | pack: %8.2f ms "
                "| GPU red: %8.2f ms | download: %8.2f ms | overhead: %8.2f ms\n",
                "", "CSC build", p.csc_build_ms, p.clearing_ms, p.submatrix_build_ms, p.gpu_pack_ms,
            ...
            if (cfg.points >= 100 && result.lockfree_profile.add_column_calls > 0)
            {
                ...
                double total_lf = lp.column_init_ms + lp.coboundary_build_ms + lp.atomics_init_ms +
                                  lp.queue_setup_ms + lp.worker_reduction_ms + lp.pair_extract_ms;
                std::snprintf(buf, sizeof(buf),
                              "  %46s  [lockfree profile: %d cols, %zu nnz, %d rows, %d threads]\n",
                              "", lp.num_columns, lp.nnz, lp.num_rows, lp.num_threads);
                write(buf);
                std::snprintf(buf, sizeof(buf),
                              "  %46s  col init: %6.2f ms | coboundary: %6.2f ms | atomics: %6.2f "
                              "ms | queue: %6.2f ms\n",
                              "", lp.column_init_ms, lp.coboundary_build_ms, lp.atomics_init_ms,
                              lp.queue_setup_ms);
                write(buf);
                std::snprintf(
                    buf, sizeof(buf),
                ...
                std::snprintf(
                    ...
                    lp.add_column_calls, lp.apparent_pairs, lp.empty_columns);
                write(buf);
            }
        }
    }
