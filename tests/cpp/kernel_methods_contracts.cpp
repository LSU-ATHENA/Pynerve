...
bool check_kernel_c_api_boundary_contract() {
    ...
    if (!throws_invalid_argument(
            [&]() { nerve::algorithms::nerve_gaussian_kernel_matrix_f64(null_diagrams, sizes, 1, 1.0, output); })) {
        return false;
    }
    if (!throws_invalid_argument(
            [&]() { nerve::algorithms::nerve_gaussian_kernel_matrix_f64(nullptr, nullptr, huge, 1.0, nullptr); })) {
        return false;
    }
    return does_not_throw([&]() {
        nerve::algorithms::nerve_gaussian_kernel_matrix_f64(nullptr, nullptr, 0, 1.0, nullptr);
        nerve::algorithms::nerve_gaussian_kernel_matrix_f64(empty_diagrams, empty_sizes, 1, 1.0, output);
    });
}
